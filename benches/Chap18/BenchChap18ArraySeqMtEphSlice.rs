...
//! Chapter 18: ArraySeq slice (multi-threaded, ephemeral) benchmark.
...
fn bench_append(c: &mut Criterion) {
    set_parallelism(10);
    let mut group = c.benchmark_group("Chap18ArraySeqMtEphSliceAppend");
...
fn bench_length(c: &mut Criterion) {
    set_parallelism(10);
    let mut group = c.benchmark_group("Chap18ArraySeqMtEphSliceLength");
    group.sample_size(10);
    group.warm_up_time(Duration::from_millis(100));
    group.measurement_time(Duration::from_millis(400));
    for &n in &[256usize, 1024] {
