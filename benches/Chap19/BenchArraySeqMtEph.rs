...
//! Chapter 19: ArraySeq (multi-threaded, ephemeral) benchmark.
...
fn bench_append(c: &mut Criterion) {
    set_parallelism(10);
    let mut group = c.benchmark_group("Chap19ArraySeqMtEphAppend");
...
fn bench_length(c: &mut Criterion) {
    set_parallelism(10);
    let mut group = c.benchmark_group("Chap19ArraySeqMtEphLength");
    group.sample_size(10);
    group.warm_up_time(Duration::from_millis(100));
    group.measurement_time(Duration::from_millis(400));
    for &n in &[256usize, 1024] {
