...
//! Chapter 37: BB-alpha BST set (multi-threaded, ephemeral) benchmark.
...
fn bench_bstset_bbalpha_mt_insert(c: &mut Criterion) {
    ...
    for &n in &[32usize, 64] {
        group.bench_with_input(BenchmarkId::new("n", n), &n, |b, &n| {
            b.iter_batched(
                || <BSTSetBBAlphaMtEph<u64> as BSTSetBBAlphaMtEphTrait<u64>>::empty(),
                |mut set| {
...
fn bench_bstset_bbalpha_mt_contains(c: &mut Criterion) {
    ...
    for &n in &[32usize, 64] {
        let mut set = <BSTSetBBAlphaMtEph<u64> as BSTSetBBAlphaMtEphTrait<u64>>::empty();
        for i in 0..n as u64 { let _ = set.insert(i); }
        group.bench_with_input(BenchmarkId::new("n", n), &n, |b, &n| {
...
criterion_group!(benches, bench_bstset_bbalpha_mt_insert, bench_bstset_bbalpha_mt_contains);
