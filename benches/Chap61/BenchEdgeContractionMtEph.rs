...
//! Chapter 61: Edge contraction (multi-threaded, ephemeral) benchmark.
...
fn bench_contract_round_mt(c: &mut Criterion) {
    set_parallelism(10);
    let mut group = c.benchmark_group("EdgeContractionMtEphRound");
    ...
    for &n in &[16usize, 32] {
        ...
        group.bench_with_input(BenchmarkId::new("n", n), &n, |b, _| {
            b.iter(|| contract_round_mt(&graph, 42));
        });
    }
    group.finish();
}

criterion_group!(benches, bench_contract_round_mt);
