...
//! Chapter 61: Edge contraction (sequential, ephemeral) benchmark.
...
fn bench_contract_round(c: &mut Criterion) {
    let mut group = c.benchmark_group("EdgeContractionStEphRound");
    ...
    for &n in &[16usize, 32] {
        ...
        group.bench_with_input(BenchmarkId::new("n", n), &n, |b, _| {
            b.iter(|| contract_round(&graph));
        });
    }
    group.finish();
}

criterion_group!(benches, bench_contract_round);
