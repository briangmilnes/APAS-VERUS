...
//! Chapter 62: Star partition (sequential, ephemeral) benchmark.
...
fn bench_sequential_star_partition(c: &mut Criterion) {
    let mut group = c.benchmark_group("StarPartitionStEph");
    ...
    for &n in &[16usize, 32] {
        ...
        group.bench_with_input(BenchmarkId::new("n", n), &n, |b, _| {
            b.iter(|| sequential_star_partition(&graph));
        });
    }
    group.finish();
}

criterion_group!(benches, bench_sequential_star_partition);
