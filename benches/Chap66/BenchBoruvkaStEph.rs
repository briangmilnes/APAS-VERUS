...
//! Chapter 66: Boruvka MST (sequential, ephemeral) benchmark.
...
fn bench_boruvka(c: &mut Criterion) {
    let mut group = c.benchmark_group("BoruvkaStEph");
    ...
    for &n in &[16usize, 32] {
        group.bench_with_input(BenchmarkId::new("chain_n", n), &n, |b, &n| {
            ...
            b.iter(|| BoruvkaStEph::boruvka_mst_with_seed(&vertices, &edges, 42));
        });
    }
    group.finish();
}

criterion_group!(benches, bench_boruvka);
