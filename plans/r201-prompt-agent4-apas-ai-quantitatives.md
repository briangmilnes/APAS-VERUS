# R201 Prompt — Agent 4: APAS-AI quantitative snapshot (cross-validation run). AFK.

## NOTE — parallel test run

This is a **test / cross-validation run** of the APAS-AI data
collection. Agent 3 is running an identical task in parallel. To
avoid filename collisions, all of your output files carry an
`-agent4` suffix. Do not touch agent3's files. After both complete,
the user will compare your numbers against agent3's to verify
consistency.
...
Produce `lectures/quantitatives/apas-ai-snapshot-agent4.md` (inside the
...
`/home/milnes/projects/APAS-VERUS-agent4/lectures/quantitatives/`
...
- `lectures/quantitatives/apas-ai-snapshot-agent4.md` — main report
- `lectures/quantitatives/raw/apas-ai-rusticate-count-loc-agent4.log`
- `lectures/quantitatives/raw/apas-ai-veracity-count-loc-agent4.log`
- `lectures/quantitatives/raw/apas-ai-git-log-agent4.txt` (optional)
...
Write `lectures/quantitatives/apas-ai-snapshot-agent4.md` with these
...
- `apas-ai-rusticate-count-loc-agent4.log`
- `apas-ai-veracity-count-loc-agent4.log`
- (Optional) `apas-ai-git-log-agent4.txt` — `git log --format='%ad %h %s' --date=short`
...
Write `plans/agent4-round201-report.md` with:
...
```
git add -A
git commit -m "R201 Agent 4: APAS-AI quantitative snapshot for lecture comparison"
git push
```
