# R72 Agent 3 — PTT Iterator Coverage (Chap37 BSTSet modules)

## Objective

Create iterator PTTs for Chap37 BSTSet modules. Each PTT proves the
...
1. `src/Chap37/BSTSetPlainMtEph.rs`
2. `src/Chap37/BSTSetRBMtEph.rs`
3. `src/Chap37/BSTSetSplayMtEph.rs`
...
2. **Read the constructor** (`new()`, `singleton()`, etc.) and `iter()`
   ...
   - What type axiom `requires` does the constructor have? These go in the
     test function's `requires`.
   - These are BSTSet wrappers around BST trees. The constructor is likely
     `new()` returning an empty set. Check `insert()` for building up the
     set — it may require wf on `&self` too.
