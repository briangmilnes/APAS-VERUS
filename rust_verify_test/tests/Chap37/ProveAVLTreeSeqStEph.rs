//! Proof tests for AVLTreeSeqStEph iterators.
//!
...
//!
//! AVLTreeSeqStEph has no IntoIterator for Self, so consume patterns are not applicable.
...
test_verify_one_file! {
    #[test] avltreeseqsteph_loop_borrow_into verus_code! {
        ...
        fn test_loop_borrow_into() {
            ...
            let mut it: AVLTreeSeqIterStEph<u64> = (&a).into_iter();
            let ghost iter_seq: Seq<u64> = it@.1;
            let ghost mut items: Seq<u64> = Seq::empty();
...
test_verify_one_file! {
    #[test] avltreeseqsteph_for_borrow_into verus_code! {
        ...
        fn test_for_borrow_into() {
            ...
            let it: AVLTreeSeqIterStEph<u64> = (&a).into_iter();
            let ghost iter_seq: Seq<u64> = it@.1;
            let ghost mut items: Seq<u64> = Seq::empty();
