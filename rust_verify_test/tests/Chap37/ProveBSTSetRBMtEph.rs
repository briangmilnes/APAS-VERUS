...
// loop-borrow-into
// r212 form C: module `BSTSetRBMtEph` defines no `IntoIterator for &BSTSetRBMtEph`; the pattern's `IntoIterator` impl is commented out
...
// loop-consume
// r212 form C: module `BSTSetRBMtEph` defines no consuming `IntoIterator for BSTSetRBMtEph`; the pattern's `IntoIterator` impl is commented out
// (src/experiments/intoiter_form_c_no_impl.rs), so this test is too.
/*
test_verify_one_file! {
