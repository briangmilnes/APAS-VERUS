//! Proof tests for RelationStEph iterators (prophetic model, r214).
//!
//! Loop patterns tested (see src/standards/iterator_ptt_standard.rs):
//!   - loop-borrow-iter:   `loop { ... r.iter() ... }`
//!   - loop-borrow-into:   `loop { ... (&r).into_iter() ... }`
//!   - for-borrow-iter:    `for x in it: r.iter()`
//!   - for-borrow-into:    `for x in it: (&r).into_iter()`
//!
