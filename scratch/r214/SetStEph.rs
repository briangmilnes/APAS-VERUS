//! Proof tests for SetStEph (prophetic iterator model, r214).
//!
...
test_verify_one_file! {
    #[test] set_st_eph_loop_loop verus_code! {
        ...
        {
            let mut s: SetStEph<u64> = SetStEph::empty(); let _ = s.insert(1); let _ = s.insert(2); let _ = s.insert(3);
            let it0 = s.iter();
...
test_verify_one_file! {
    #[test] set_st_eph_for_iter verus_code! {
        ...
        fn test_body()
            requires valid_key_type::<u64>()
        {
            let mut s: SetStEph<u64> = SetStEph::empty(); let _ = s.insert(10); let _ = s.insert(20); let _ = s.insert(30);
            let it0 = s.iter();
            let ghost orig: Seq<u64> = into_iter_hash_keys(it0);
            let mut collected: Vec<u64> = Vec::new();
            for x in it: it0
