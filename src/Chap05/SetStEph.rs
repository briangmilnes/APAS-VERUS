...
verus! {
    ...
    pub struct SetStEph<T: StT + Hash> { pub data: HashSetWithViewPlus<T> }
...
/*
    ...
    impl<'a, T: StT + Hash> Iterator for SetStEphIter<'a, T> {
        ...
        fn next(&mut self) -> (result: Option<Self::Item>)
            ensures
                ({ let (old_index, old_seq) = old(self).inner@; match result { None => { &&& self.inner@ == old(self).inner@ &&& old_index >= old_seq.len() },
                Some(element) => { let (new_index, new_seq) = self.inner@; &&& 0 <= old_index < old_seq.len() &&& new_seq == old_seq &&& new_index == old_index + 1 &&& element == old_seq[old_index] },
                } }),
            { self.inner.next() }
        }
        ...
        impl<'a, T: StT + Hash> vstd::pervasive::ForLoopGhostIteratorNew for SetStEphIter<'a, T> {
            ...
            open spec fn ghost_iter(&self) -> SetStEphIterGhostIterator<'a, T> {
                SetStEphIterGhostIterator { pos: self.inner@.0, elements: self.inner@.1, phantom: None }
            }
        }
