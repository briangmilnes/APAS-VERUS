            ...
            loop
                invariant
                    ...
                    forall |w: V::V| #[trigger] ng@.contains(w) <==>
                        exists |i: int| #![trigger le_seq[i]] 0 <= i < pos &&
                                ...
                                if ng@.contains(w) {
                                    let i = choose |i: int| #![trigger le_seq[i]] 0 <= i < le_seq.len() &&
                                        ((le_seq[i]@.0 == v_view && le_seq[i]@.1 == w) ||
                                         (le_seq[i]@.1 == v_view && le_seq[i]@.0 == w));
                                    lemma_seq_index_in_map_to_set(le_seq, i);
                                    if le_seq[i]@.0 == v_view && le_seq[i]@.1 == w {
                                        assert(le_view.contains((v_view, w, le_seq[i]@.2)));
                                    } else {
                                        assert(le_view.contains((w, v_view, le_seq[i]@.2)));
                                    }
                                }
                            }
                            // Veracity: NEEDED assert
