...
//! Chapter 6 Weighted Directed Graph (ephemeral) with i128 weights - Single-threaded version.
//! Uses CheckedI128 for overflow-safe weight summation.
...
pub mod WeightedDirGraphStEphI128 {
    ...
    use crate::vstdplus::checked_int::checked_int::CheckedI128;
...
verus! {
    ...
    pub type WeightedDirGraphStEphI128<V> = LabDirGraphStEph<V, i128>;

    pub trait WeightedDirGraphStEphI128Trait<V: StT + Hash>: 
        View<V = LabGraphView<<V as View>::V, i128>> + Sized {

        open spec fn spec_total_weight(&self) -> int 
         { self@.A.fold(0int, |acc: int, t: (V::V, V::V, i128)| acc + t.2 as int) }
    ...
    impl<V: StT + Hash> WeightedDirGraphStEphI128Trait<V> for WeightedDirGraphStEphI128<V> {
        ...
        fn weighed_edges(&self) -> (weighed_edges: SetStEph<WeightedEdge<V, i128>>) {
            ...
            loop
                invariant
                    ...
                    it@.0 <= wa_seq.len(),
                    it@.1 == wa_seq,
                    wa_seq.map(|i: int, e: LabEdge<V, i128>| e@).to_set() == wa_view,
                    forall |t: (V::V, V::V, i128)| edges@.contains(t) == 
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, i128)| #[trigger] edges@.contains(t) implies wa_view.contains(t) by {
                                if edges@.contains(t) {
...
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, i128)| #[trigger] wa_view.contains(t) implies edges@.contains(t) by {
        ...
        fn out_neighbors_weighed(&self, v: &V) -> (out_neighbors: SetStEph<Pair<V, i128>>) {
            ...
            loop
                invariant
                    ...
                    it@.1 == wa_seq,
                    wa_seq.map(|i: int, e: LabEdge<V, i128>| e@).to_set() == wa_view,
                    forall |p: (V::V, i128)| neighbors@.contains(p) == 
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
// Veracity: UNNEEDED assert                             assert forall |p: (V::V, i128)| neighbors@.contains(p) implies 
...
// Veracity: UNNEEDED assert                             assert forall |p: (V::V, i128)| (exists |w: i128| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w) implies 
                                neighbors@.contains(p) by {
                                if exists |w: i128| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w {
                                    let w = choose |w: i128| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w;
...
// Veracity: UNNEEDED assert                             assert forall |p: (V::V, i128)| (exists |w: i128| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w) implies 
                                neighbors@.contains(p) by {
                                if exists |w: i128| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w {
                                    let w = choose |w: i128| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w;
        ...
        fn total_weight(&self) -> (total_weight: CheckedI128) { 
            ...
            loop
                invariant
                    ...
                    wa_seq.map(|i: int, e: LabEdge<V, i128>| e@).to_set() == wa_view,
                    sum@ == wa_seq.take(it@.0 as int).fold_left(0int, |acc: int, e: LabEdge<V, i128>| acc + e@.2 as int),
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
                            lemma_signed_seq_fold_left_plus_is_weighted_seq_sum_i128::<LabEdge<V, i128>, V::V, V::V>(wa_seq);
                            lemma_signed_weighted_seq_fold_equals_set_fold_i128(wa_seq.map(|_i: int, e: LabEdge<V, i128>| e@));
                        }
                        return sum;
                    },
                    Some(labeled_edge) => {
        ...
        fn edges_above_weight(&self, threshold: i128) -> (edges_above: SetStEph<WeightedEdge<V, i128>>) {
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, i128)| #[trigger] edges@.contains(t) implies 
...
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, i128)| #[trigger] wa_view.contains(t) && t.2 > threshold implies 
                                edges@.contains(t) by {
...
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, i128)| #[trigger] wa_view.contains(t) && t.2 < threshold implies 
    ...
    #[macro_export]
    macro_rules! WeightedDirGraphStEphI128Lit {
        () => {{
        ...
        ( V: [ $( $v:expr ),* $(,)? ], E: [ $( $edge:expr ),* $(,)? ] ) => {{
            ...
            <$crate::Chap06::WeightedDirGraphStEphI128::WeightedDirGraphStEphI128::WeightedDirGraphStEphI128<_> as $crate::Chap06::WeightedDirGraphStEphI128::WeightedDirGraphStEphI128::WeightedDirGraphStEphI128Trait<_>>::from_weighed_edges(vertices, edges)
        }};
    }
}
