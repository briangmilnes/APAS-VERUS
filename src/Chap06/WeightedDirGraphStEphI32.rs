...
verus! 
{
    ...
    pub trait WeightedDirGraphStEphI32Trait<V: StT + Hash>: 
        ...
        fn get_edge_weight(&self, from: &V, to: &V) -> (weight: Option<i32>)
            requires spec_labgraphview_wf(self@), valid_key_type_WeightedEdge::<V, i32>()
            ensures
        ...
        fn total_weight(&self) -> (total_weight: CheckedI32) { 
            ...
            loop
                invariant
                    ...
                    wa_seq.map(|i: int, e: LabEdge<V, i32>| e@).to_set() == wa_view,
                    sum@ == wa_seq.take(pos).fold_left(0int, |acc: int, e: LabEdge<V, i32>| acc + e@.2 as int),
                decreases IteratorSpec::decrease(&it)->0,
            // Veracity: NEEDED proof block
            {
                let ghost old_pos = pos;
                match it.next() {
                    None => {
                        // Veracity: NEEDED proof block
                        proof {
                            lemma_reveal_view_injective::<LabEdge<V, i32>>();
