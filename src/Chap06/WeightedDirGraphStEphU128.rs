        ...
        fn from_weighed_edges(vertices: SetStEph<V>, edges: SetStEph<WeightedEdge<V, u128>>) -> (g: WeightedDirGraphStEphU128<V>) {
            ...
            loop
                invariant
                    ...
                    edge_seq.map(|i: int, e: WeightedEdge<V, u128>| e@).to_set() == edges@,
                    forall |u: V::V, w: V::V, weight: u128| 
        ...
        fn total_weight(&self) -> (total_weight: CheckedU128) { 
            ...
            loop
                invariant
                    ...
                    wa_seq.map(|i: int, e: LabEdge<V, u128>| e@).to_set() == wa_view,
                    sum@ == wa_seq.take(pos).fold_left(0int, |acc: int, e: LabEdge<V, u128>| acc + e@.2 as nat),
                decreases IteratorSpec::decrease(&it)->0,
            // Veracity: NEEDED proof block
            {
                let ghost old_pos = pos;
                match it.next() {
                    None => {
                        // Veracity: NEEDED proof block
                        proof {
                            lemma_reveal_view_injective::<LabEdge<V, u128>>();
