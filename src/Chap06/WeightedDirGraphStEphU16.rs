...
verus! {
    ...
    pub trait WeightedDirGraphStEphU16Trait<V: StT + Hash>: 
        ...
        fn get_edge_weight(&self, from: &V, to: &V) -> (weight: Option<u16>)
            requires spec_labgraphview_wf(self@), valid_key_type_WeightedEdge::<V, u16>()
            ensures
        ...
        fn weighed_edges(&self) -> (weighed_edges: SetStEph<WeightedEdge<V, u16>>) {
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
assert forall |t: (V::V, V::V, u16)| #[trigger] edges@.contains(t) implies wa_view.contains(t) by {
                                if edges@.contains(t) {
                                    let i = choose |i: int| #![trigger wa_seq[i]] 0 <= i < wa_seq.len() && wa_seq[i]@ == t;
                                    lemma_seq_index_in_map_to_set(wa_seq, i);
                                }
                            }
assert forall |t: (V::V, V::V, u16)| #[trigger] wa_view.contains(t) implies edges@.contains(t) by {
        ...
        fn out_neighbors_weighed(&self, v: &V) -> (out_neighbors: SetStEph<Pair<V, u16>>) {
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
assert forall |p: (V::V, u16)| neighbors@.contains(p) implies 
...
assert forall |p: (V::V, u16)| (exists |w: u16| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w) implies 
...
assert forall |p: (V::V, u16)| (exists |w: u16| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w) implies 
                                neighbors@.contains(p) by {
        ...
        fn edges_above_weight(&self, threshold: u16) -> (edges_above: SetStEph<WeightedEdge<V, u16>>) {
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
assert forall |t: (V::V, V::V, u16)| #[trigger] edges@.contains(t) implies 
...
assert forall |t: (V::V, V::V, u16)| #[trigger] wa_view.contains(t) && t.2 > threshold implies 
                                edges@.contains(t) by {
