        ...
        fn from_weighed_edges(vertices: SetStEph<V>, edges: SetStEph<WeightedEdge<V, u64>>) -> (g: WeightedDirGraphStEphU64<V>) {
            ...
            loop
                invariant
                    ...
                    it@.0 <= edge_seq.len(),
                    it@.1 == edge_seq,
                    edge_seq.map(|i: int, e: WeightedEdge<V, u64>| e@).to_set() == edges@,
                    forall |u: V::V, w: V::V, weight: u64| 
        ...
        fn out_neighbors_weighed(&self, v: &V) -> (out_neighbors: SetStEph<Pair<V, u64>>) {
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
assert forall |p: (V::V, u64)| neighbors@.contains(p) implies 
...
assert forall |p: (V::V, u64)| (exists |w: u64| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w) implies 
...
assert forall |p: (V::V, u64)| (exists |w: u64| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w) implies 
                                neighbors@.contains(p) by {
        ...
        fn total_weight(&self) -> (total_weight: CheckedU64) { 
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
                            lemma_reveal_view_injective::<LabEdge<V, u64>>();
        ...
        fn edges_above_weight(&self, threshold: u64) -> (edges_above: SetStEph<WeightedEdge<V, u64>>) {
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
assert forall |t: (V::V, V::V, u64)| #[trigger] edges@.contains(t) implies 
...
assert forall |t: (V::V, V::V, u64)| #[trigger] wa_view.contains(t) && t.2 > threshold implies 
                                edges@.contains(t) by {
