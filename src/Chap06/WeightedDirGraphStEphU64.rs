...
//! Chapter 6 Weighted Directed Graph (ephemeral) with u64 weights - Single-threaded version.
//! Uses CheckedU64 for overflow-safe weight summation.
...
pub mod WeightedDirGraphStEphU64 {
    ...
    use crate::vstdplus::checked_nat::checked_nat::CheckedU64;
...
verus! {
    ...
    pub type WeightedDirGraphStEphU64<V> = LabDirGraphStEph<V, u64>;

    pub trait WeightedDirGraphStEphU64Trait<V: StT + Hash>: 
        View<V = LabGraphView<<V as View>::V, u64>> + Sized {

        open spec fn spec_total_weight(&self) -> nat 
         { self@.A.fold(0nat, |acc: nat, t: (V::V, V::V, u64)| acc + t.2 as nat) }
    ...
    impl<V: StT + Hash> WeightedDirGraphStEphU64Trait<V> for WeightedDirGraphStEphU64<V> {
        ...
        fn weighed_edges(&self) -> (weighed_edges: SetStEph<WeightedEdge<V, u64>>) {
            ...
            loop
                invariant
                    ...
                    it@.0 <= wa_seq.len(),
                    it@.1 == wa_seq,
                    wa_seq.map(|i: int, e: LabEdge<V, u64>| e@).to_set() == wa_view,
                    forall |t: (V::V, V::V, u64)| edges@.contains(t) == 
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, u64)| #[trigger] edges@.contains(t) implies wa_view.contains(t) by {
                                if edges@.contains(t) {
...
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, u64)| #[trigger] wa_view.contains(t) implies edges@.contains(t) by {
        ...
        fn out_neighbors_weighed(&self, v: &V) -> (out_neighbors: SetStEph<Pair<V, u64>>) {
            ...
            loop
                invariant
                    ...
                    it@.1 == wa_seq,
                    wa_seq.map(|i: int, e: LabEdge<V, u64>| e@).to_set() == wa_view,
                    forall |p: (V::V, u64)| neighbors@.contains(p) == 
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
// Veracity: UNNEEDED assert                             assert forall |p: (V::V, u64)| neighbors@.contains(p) implies 
                                (exists |w: u64| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w) by {
                                if neighbors@.contains(p) {
                                    let i = choose |i: int| #![trigger wa_seq[i]] 0 <= i < wa_seq.len() && wa_seq[i]@.0 == v_view && wa_seq[i]@.1 == p.0 && wa_seq[i]@.2 == p.1;
                                    crate::vstdplus::seq_set::lemma_seq_index_in_map_to_set(wa_seq, i);
                                }
                            }
// Veracity: UNNEEDED assert                             assert forall |p: (V::V, u64)| (exists |w: u64| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w) implies 
                                neighbors@.contains(p) by {
                                if exists |w: u64| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w {
                                    let w = choose |w: u64| #![trigger wa_view.contains((v_view, p.0, w))] wa_view.contains((v_view, p.0, w)) && p.1 == w;
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
// Veracity: UNNEEDED assert                             assert forall |p: (V::V, u64)| neighbors@.contains(p) implies 
                                (exists |w: u64| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w) by {
                                if neighbors@.contains(p) {
                                    let i = choose |i: int| #![trigger wa_seq[i]] 0 <= i < wa_seq.len() && wa_seq[i]@.1 == v_view && wa_seq[i]@.0 == p.0 && wa_seq[i]@.2 == p.1;
                                    crate::vstdplus::seq_set::lemma_seq_index_in_map_to_set(wa_seq, i);
                                }
                            }
// Veracity: UNNEEDED assert                             assert forall |p: (V::V, u64)| (exists |w: u64| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w) implies 
                                neighbors@.contains(p) by {
                                if exists |w: u64| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w {
                                    let w = choose |w: u64| #![trigger wa_view.contains((p.0, v_view, w))] wa_view.contains((p.0, v_view, w)) && p.1 == w;
        ...
        fn total_weight(&self) -> (total_weight: CheckedU64) { 
            ...
            loop
                invariant
                    ...
                    wa_seq.map(|i: int, e: LabEdge<V, u64>| e@).to_set() == wa_view,
                    sum@ == wa_seq.take(it@.0 as int).fold_left(0int, |acc: int, e: LabEdge<V, u64>| acc + e@.2 as nat),
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
                            lemma_seq_fold_left_plus_is_weighted_seq_sum_u64::<LabEdge<V, u64>, V::V, V::V>(wa_seq);
                            lemma_fold_left_int_equals_nat_as_int_u64::<LabEdge<V, u64>, V::V, V::V>(wa_seq);
                            lemma_weighted_seq_fold_equals_set_fold_u64(wa_seq.map(|_i: int, e: LabEdge<V, u64>| e@));
                        }
                        return sum;
                    },
                    Some(labeled_edge) => {
        ...
        fn edges_above_weight(&self, threshold: u64) -> (edges_above: SetStEph<WeightedEdge<V, u64>>) {
                ...
                decreases wa_seq.len() - it@.0,
            {
                match it.next() {
                    None => {
                        proof {
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, u64)| #[trigger] edges@.contains(t) implies 
...
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, u64)| #[trigger] wa_view.contains(t) && t.2 > threshold implies 
                                edges@.contains(t) by {
...
// Veracity: UNNEEDED assert                             assert forall |t: (V::V, V::V, u64)| #[trigger] wa_view.contains(t) && t.2 < threshold implies 
    ...
    #[macro_export]
    macro_rules! WeightedDirGraphStEphU64Lit {
        () => {{
        ...
        ( V: [ $( $v:expr ),* $(,)? ], E: [ $( $edge:expr ),* $(,)? ] ) => {{
            ...
            <$crate::Chap06::WeightedDirGraphStEphU64::WeightedDirGraphStEphU64::WeightedDirGraphStEphU64<_> as $crate::Chap06::WeightedDirGraphStEphU64::WeightedDirGraphStEphU64::WeightedDirGraphStEphU64Trait<_>>::from_weighed_edges(vertices, edges)
        }};
    }
}
