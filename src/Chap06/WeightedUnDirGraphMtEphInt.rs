...
pub mod WeightedUnDirGraphMtEphInt {
    ...
    impl<V: HashOrd + MtT + 'static> WeightedUnDirGraphMtEphIntTrait<V> for WeightedUnDirGraphMtEphInt<V> {
        ...
        fn neighbors_weighted(&self, v: &V) -> SetStEph<Pair<V, i32>> {
            ...
            fn parallel_neighbors<V: HashOrd + MtT + 'static>(
                ...
                if n == 1 {
                    if edges[0].0 == v {
                        ...
                        s.insert(Pair(edges[0].1.clone(), edges[0].2));
                        return s;
                    } else if edges[0].1 == v {
                        let mut s = SetStEph::empty();
                        s.insert(Pair(edges[0].0.clone(), edges[0].2));
                        return s;
                    }
                    return SetStEph::empty();
                }
            ...
            parallel_neighbors(edges, v.clone())
        }
        ...
        fn vertex_degree(&self, v: &V) -> usize { self.neighbors_weighted(v).size() }
    }
