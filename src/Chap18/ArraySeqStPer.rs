...
pub mod ArraySeqStPer {
    ...
    impl<T: View> ArraySeqStPerS<T> {
        ...
        {
            ArraySeqStPerS { data: vec![init_value; length] }
        }
    ...
    impl<T: Clone> Clone for ArraySeqStPerS<T> {
        fn clone(&self) -> Self { ArraySeqStPerS { data: self.data.clone() } }
    }
    ...
    impl<T> ArraySeqStPerS<T> {
        ...
        pub fn empty() -> Self { ArraySeqStPerS { data: Vec::new() } }
        pub fn singleton(item: T) -> Self { ArraySeqStPerS { data: vec![item] } }
        pub fn tabulate<F: Fn(usize) -> T>(f: &F, length: usize) -> Self {
            ArraySeqStPerS { data: (0..length).map(f).collect() }
        }
        pub fn map<U, F: Fn(&T) -> U>(a: &Self, f: &F) -> ArraySeqStPerS<U> {
            ArraySeqStPerS { data: a.data.iter().map(f).collect() }
        }
        ...
        pub fn filter<F: Fn(&T) -> bool>(a: &Self, pred: &F) -> Self where T: Clone {
            ArraySeqStPerS { data: a.data.iter().filter(|x| pred(x)).cloned().collect() }
        }
        pub fn isEmpty(&self) -> bool { self.data.is_empty() }
        pub fn isSingleton(&self) -> bool { self.data.len() == 1 }
        pub fn from_vec(elts: Vec<T>) -> Self { ArraySeqStPerS { data: elts } }
        ...
        pub fn subseq_copy(&self, start: usize, length: usize) -> Self where T: Clone {
            ...
            ArraySeqStPerS { data: self.data[start..end].to_vec() }
        }
