...
pub mod ArraySeqStPer {
    ...
    pub trait ArraySeqStPerRedefinableTrait<T>: ArraySeqStPerBaseTrait<T> {
        ...
        /// - Work Θ(|a|), Span Θ(1).
        fn reduce<F: Fn(&T, &T) -> T>(a: &ArraySeqStPerS<T>, f: &F, Ghost(spec_f): Ghost<spec_fn(T, T) -> T>, id: T) -> (result: T)
            where T: Clone
            requires
    ...
    impl<T> ArraySeqStPerRedefinableTrait<T> for ArraySeqStPerS<T> {
        ...
        fn iterate<A, F: Fn(&A, &T) -> A>(a: &ArraySeqStPerS<T>, f: &F, Ghost(spec_f): Ghost<spec_fn(A, T) -> A>, seed: A) -> (acc: A) {
