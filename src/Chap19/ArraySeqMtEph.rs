...
// Veracity: UNNEEDED proof block                 let a_last = s[n];
// Veracity: UNNEEDED proof block 
// Veracity: UNNEEDED proof block                 s.lemma_fold_left_split(id, f, n);
                ...
                forall|i: int| 0 <= i < b.seq@.len() ==> concatenated.seq@[a.seq@.len() + i] == b.seq@[i],
        {
            ...
            while j < b_len
                invariant
                    ...
                    IteratorSpec::remaining(&iter).len() == b_view.len() - j,
                    forall|i: int| 0 <= i < IteratorSpec::remaining(&iter).len()
                        ==> #[trigger] IteratorSpec::remaining(&iter)[i] == b_view[j + i],
