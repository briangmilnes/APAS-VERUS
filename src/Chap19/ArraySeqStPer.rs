...
pub mod ArraySeqStPer {
    ...
    impl<T: View + Clone> ArraySeqStPerS<T> {
        ...
        // Since length is 0, the closure is never called.
        pub fn empty19() -> (result: ArraySeqStPerS<T>)
        ...
        pub fn singleton19(item: T) -> (result: ArraySeqStPerS<T>)
        ...
        pub fn map19<U: Clone + View, F: Fn(&T) -> U>(a: &ArraySeqStPerS<T>, f: &F) -> (result: ArraySeqStPerS<U>)
            requires
        ...
        #[verifier::external_body]
        pub fn append19(a: &ArraySeqStPerS<T>, b: &ArraySeqStPerS<T>) -> (result: ArraySeqStPerS<T>)
            requires a.seq@.len() + b.seq@.len() <= usize::MAX as int
            ensures result.seq@.len() == a.seq@.len() + b.seq@.len()
        {
            Self::flatten19(&ArraySeqStPerS::<ArraySeqStPerS<T>>::tabulate(
                &|i: usize| if i == 0 { a.clone() } else { b.clone() },
                2,
            ))
        }

        // Helper: flatten for append19
        #[verifier::external_body]
        fn flatten19(ss: &ArraySeqStPerS<ArraySeqStPerS<T>>) -> (result: ArraySeqStPerS<T>)
        {
        ...
        #[verifier::external_body]
        pub fn filter19<F: Fn(&T) -> bool>(a: &ArraySeqStPerS<T>, pred: &F) -> (result: ArraySeqStPerS<T>)
            requires
                a.seq@.len() <= usize::MAX as int,
                forall|i: int| 0 <= i < a.seq@.len() ==> #[trigger] pred.requires((&a.seq@[i],)),
            ensures result.seq@.len() <= a.seq@.len()
        {
            let deflated: ArraySeqStPerS<ArraySeqStPerS<T>> = ArraySeqStPerS::<ArraySeqStPerS<T>>::tabulate(
                &|i: usize| Self::deflate(pred, a.nth(i)),
                a.length(),
            );
            Self::flatten19(&deflated)
        }

        // Helper: deflate for filter19
        fn deflate<F: Fn(&T) -> bool>(pred: &F, x: &T) -> (result: ArraySeqStPerS<T>)
        ...
        #[verifier::external_body]
        pub fn update19(a: &ArraySeqStPerS<T>, index: usize, item: T) -> (result: ArraySeqStPerS<T>)
            requires
                index < a.seq@.len(),
                a.seq@.len() <= usize::MAX as int,
            ensures result.seq@.len() == a.seq@.len()
        ...
        pub fn isEmpty19(a: &ArraySeqStPerS<T>) -> (empty: bool)
        ...
        pub fn isSingleton19(a: &ArraySeqStPerS<T>) -> (single: bool)
        ...
        pub fn iterate19<A, F: Fn(&A, &T) -> A>(a: &ArraySeqStPerS<T>, f: &F, seed: A) -> (result: A)
        ...
        pub fn reduce19<F: Fn(&T, &T) -> T>(a: &ArraySeqStPerS<T>, f: &F, id: T) -> (result: T)
            requires
                a.seq@.len() <= usize::MAX as int,
            ...
            decreases a.seq@.len()
        {
            ...
            if len == 0 {
                ...
                let left_result = Self::reduce19(&left, f, id.clone());
                let right_result = Self::reduce19(&right, f, id);
                f(&left_result, &right_result)
            }
        }

        // Algorithm 19.10: scan using contraction (simplified sequential version)
        pub fn scan19<F: Fn(&T, &T) -> T>(a: &ArraySeqStPerS<T>, f: &F, id: T) -> (result: (ArraySeqStPerS<T>, T))
        ...
        // select helper for append_select19
        pub fn select19<'a>(a: &'a ArraySeqStPerS<T>, b: &'a ArraySeqStPerS<T>, i: usize) -> (result: Option<&'a T>)
            ensures
        ...
        #[verifier::external_body]
        pub fn append_select19(a: &ArraySeqStPerS<T>, b: &ArraySeqStPerS<T>) -> (result: ArraySeqStPerS<T>)
            requires a.seq@.len() + b.seq@.len() <= usize::MAX as int
            ensures result.seq@.len() == a.seq@.len() + b.seq@.len()
        {
            let total = a.length() + b.length();
            Self::tabulate(&|i: usize| Self::select19(a, b, i).unwrap().clone(), total)
        }
    }
    ...
    impl<T: Clone> ArraySeqStPerS<T> {
        pub fn empty19() -> Self { Self::from_vec(Vec::new()) }
        pub fn singleton19(item: T) -> Self { Self::from_vec(vec![item]) }
        pub fn map19<U: Clone, F: Fn(&T) -> U>(a: &Self, f: &F) -> ArraySeqStPerS<U> {
            ArraySeqStPerS::<U>::tabulate(&|i| f(a.nth(i)), a.length())
        }
        pub fn append19(a: &Self, b: &Self) -> Self {
            let mut seq = a.seq.clone();
            seq.extend(b.seq.iter().cloned());
            Self { seq }
        }
        fn flatten19(ss: &ArraySeqStPerS<ArraySeqStPerS<T>>) -> Self {
        ...
        pub fn filter19<F: Fn(&T) -> bool>(a: &Self, pred: &F) -> Self {
            Self { seq: a.seq.iter().filter(|x| pred(x)).cloned().collect() }
        }
        fn deflate<F: Fn(&T) -> bool>(pred: &F, x: &T) -> Self {
            if pred(x) { Self::singleton(x.clone()) } else { Self::empty() }
        }
        pub fn update19(a: &Self, index: usize, item: T) -> Self {
            Self::tabulate(&|j| if j == index { item.clone() } else { a.nth(j).clone() }, a.length())
        }
        pub fn isEmpty19(a: &Self) -> bool { a.length() == 0 }
        pub fn isSingleton19(a: &Self) -> bool { a.length() == 1 }
        pub fn iterate19<A, F: Fn(&A, &T) -> A>(a: &Self, f: &F, seed: A) -> A {
            a.seq.iter().fold(seed, |acc, x| f(&acc, x))
        }
        pub fn reduce19<F: Fn(&T, &T) -> T>(a: &Self, f: &F, id: T) -> T {
            if a.length() == 0 { id }
                ...
                f(&Self::reduce19(&left, f, id.clone()), &Self::reduce19(&right, f, id))
            }
        }
        pub fn scan19<F: Fn(&T, &T) -> T>(a: &Self, f: &F, id: T) -> (Self, T) {
            let mut acc = id;
            let seq: Vec<T> = a.seq.iter().map(|x| { acc = f(&acc, x); acc.clone() }).collect();
            (Self { seq }, acc)
        }
        pub fn select19<'a>(a: &'a Self, b: &'a Self, i: usize) -> Option<&'a T> {
            if i < a.length() { Some(a.nth(i)) }
            else if i - a.length() < b.length() { Some(b.nth(i - a.length())) }
            else { None }
        }
        pub fn append_select19(a: &Self, b: &Self) -> Self {
            Self::tabulate(&|i| Self::select19(a, b, i).unwrap().clone(), a.length() + b.length())
        }
    }
}
