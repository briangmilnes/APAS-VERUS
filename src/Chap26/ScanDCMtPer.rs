    ...
    verus! 
{
    ...
    pub trait ScanDCMtTrait {
        ...
        /// - Alg Analysis: Code review (Claude Opus 4.6): ACCEPTED DIFFERENCE: Work O(n lg n), Span O(n) — parallel recursion via join(), sequential O(n) combine: S(n) = S(n/2) + O(n) = O(n).
