...
| 14 | ETSPMtPer | `etsp_parallel` | **weak** | `external_body` | Same spec as St: `tour.len() == points.len()` |
| 15 | ETSPMtPer | `etsp_parallel_inner` | none | — | Outside verus!, no spec (f64 + ParaPair!) |
...
| 5 | `etsp_parallel` | ETSPMtPer | Θ(lg² n) | **Θ(n²)** | Recursive calls are parallel via ParaPair!, but O(n²) swap search is sequential |
...
| 7 | ETSPStPer | Algorithm 26.7 | **Mostly** | Adds n=3 base case (greedy nearest neighbor) not in prose. Uses sort-based median split (prose says "find median along longest dimension"). Sequential only. |
| 8 | ETSPMtPer | Algorithm 26.7 (parallel) | **Partial** | Recursive calls parallelized via `ParaPair!` (help-first scheduler). Swap search (`minVal`) is sequential O(n²) — APAS achieves Θ(lg n) span with parallel reduce over edge pairs. |
...
| 10 | `etsp_parallel` | ETSPMtPer | **Partial** | `ParaPair!` for recursive calls; swap search sequential |
| 11 | `etsp_parallel_inner` | ETSPMtPer | **Partial** | Same — recursive fork-join parallel, O(n²) combine sequential |
...
| 8 | ETSPMtPer | TestETSPMtPer.rs | 1 | 7 | etsp_parallel tested (mirrors St tests) |
...
| 8 | ETSPMtPer | Yes | 2-pt, 3-pt, collinear | Tour length quality check | N/A | **Excellent** |
...
| 3 | ~~eTSP parallel variant~~ | ~~Not implemented~~ | **IMPLEMENTED (2026-02-13):** `ETSPMtPer` module added with `ParaPair!` for recursive fork-join. Swap search still sequential O(n²). |
...
| 11 | `etsp_parallel_inner` | fn | ETSPMtPer | Parallel recursive helper, outside verus! (f64 + ParaPair!) |
...
| 6 | ETSPStPer | `etsp` | Delegates to `etsp_inner` (outside verus!, uses f64) | **f64 (permanent)** |
| 7 | ETSPMtPer | `etsp_parallel` | Delegates to `etsp_parallel_inner` (f64 + ParaPair!) | **f64 + threading (permanent)** |
