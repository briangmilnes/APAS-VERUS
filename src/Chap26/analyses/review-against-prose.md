...
| 11 | ETSPStPer | `etsp` | **weak** | `external_body` | Only ensures `tour.len() == points.len()` |
| 12 | ETSPStPer | `distance` | none | — | Outside verus!, no spec |
| 13 | ETSPStPer | `etsp_inner` | none | — | Outside verus!, no spec |
| 14 | MergeSortMtPer | `merge_parallel` | strong | `external_body` | spec_merge_post (sorted + permutation) |
| 15 | MergeSortMtPer | `merge_sort_parallel` | strong | `external_body` | spec_sort_post (sorted + permutation) |
| 16 | MergeSortStPer | `merge` | strong | — | spec_merge_post — fully verified |
| 17 | MergeSortStPer | `merge_sort` | strong | — | spec_sort_post — fully verified |
| 18 | MergeSortStPer | `lemma_push_sorted` | strong | — | Proof fn — fully verified |
| 19 | ScanDCMtPer | `prefix_sums_dc_parallel` | strong | `external_body` | spec_scan_post |
| 20 | ScanDCMtPer | `prefix_sums_dc_inner` | none | — | Outside verus!, no spec |
| 21 | ScanDCStPer | `scan_dc` | strong | — | spec_scan_post — fully verified |
| 22 | ScanDCStPer | `prefix_sums_dc` | strong | `external_body` | Delegates to scan_dc but closure unverified |
| 23 | ScanDCStPer | `lemma_fold_left_monoid` | strong | — | Proof fn — fully verified |

**Summary:** 19 strong, 1 weak, 3 none. 9 proof holes (all `external_body`).
...
|---|----------|--------|-----------|-------------|--------|
| 1 | `max_element_parallel` | DivConReduceMtPer | Θ(lg n) | **Θ(n)** | Sequential while loop, NOT parallel despite Mt module name |
...
| 5 | All St functions | All St modules | Θ(lg n) or Θ(lg² n) | **Span = Work** | Sequential implementations; span equals work by definition |
...
| 2 | DivConReduceMtPer | Algorithm 26.2 (parallel) | **Partial** | `max_element_parallel` is a sequential while loop — should use parallel reduce. Other functions correctly delegate to `ArraySeqMtPerS::reduce` |
...
| 7 | ETSPStPer | Algorithm 26.7 | **Mostly** | Adds n=3 base case (greedy nearest neighbor) not in prose. Uses sort-based median split (prose says "find median along longest dimension"). Sequential only — **no Mt variant exists** |
...
| 10 | `etsp` | Tour visits all points, forms cycle | **No** — only ensures `tour.len() == points.len()` | Missing: edges form valid cycle, all points visited exactly once, edges connect actual input points |

**Summary:** 9/10 functions have strong spec fidelity. `etsp` has a **weak** spec that only captures the length property, not structural validity or correctness. This is expected given the eTSP implementation is entirely outside `verus!` (uses `f64`).
...
|---|----------|--------|---------------|-----------|
| 1 | `max_element_parallel` | DivConReduceMtPer | **Sequential** | While loop, no threading |
...
|---|----------|-----------|-------------|--------|-------|
| 1 | `max_element_parallel` | Θ(lg n) | Θ(n) | **NO** | Aspirational — sequential while loop |
...
|---|----------|-----------|--------|-----------|-------|
| 1 | `max_element_parallel` | Θ(lg n) | Θ(n) | **NO** | Should call `ArraySeqMtPerS::reduce` like `sum_parallel` does |
...
| 7 | ETSPStPer | TestETSPStPer.rs | 1 | 7 | etsp tested extensively |
...
| 7 | ETSPStPer | Yes | 2-pt, 3-pt, collinear | Tour length quality check | N/A | **Excellent** |
...
| 3 | eTSP parallel variant | Not implemented | Only `ETSPStPer` exists. No `ETSPMtPer` module. APAS gives Span Θ(lg² n) implying parallelism. |
...
| 4 | `spec_is_valid_tour` | spec fn | ETSPStPer | Trivial tour validity (n_edges == n_points) |
...
| 7 | `etsp_inner` | fn | ETSPStPer | Recursive helper, outside verus! |
| 8 | `prefix_sums_dc_inner` | fn | ScanDCMtPer | Parallel recursive helper, outside verus! |
| 9 | `Point`, `Edge` types | struct | ETSPStPer | Data definitions implied by prose |
| 10 | `distance` | fn | ETSPStPer | Euclidean distance, implied by ‖u − v‖ in prose |

---

## Proof Holes Summary

9 `external_body` holes across 6 modules:

| # | Module | Function | Reason for Hole |
|---|--------|----------|----------------|
| 1 | DivConReduceStPer | `sum` | usize overflow: spec wraps via `as N`, exec checks overflow |
| 2 | DivConReduceStPer | `product` | Same usize overflow issue |
| 3 | DivConReduceMtPer | `sum_parallel` | Same usize overflow + parallel delegation |
| 4 | DivConReduceMtPer | `product_parallel` | Same usize overflow + parallel delegation |
| 5 | ETSPStPer | `etsp` | Delegates to `etsp_inner` (outside verus!, uses f64) |
| 6 | MergeSortMtPer | `merge_parallel` | Parallel merge with thread::scope — Verus cannot verify threading |
| 7 | MergeSortMtPer | `merge_sort_parallel` | Parallel sort with ParaPair! — Verus cannot verify threading |
| 8 | ScanDCMtPer | `prefix_sums_dc_parallel` | Delegates to parallel inner function — Verus cannot verify threading |
| 9 | ScanDCStPer | `prefix_sums_dc` | Closure `|x, y| x + y` cannot satisfy biconditional ensures for usize |

**Categorized:**
- **usize overflow** (4 holes): sum, product (St + Mt) — fixable with `wrapping_add`/`wrapping_mul`
- **Threading** (3 holes): merge_parallel, merge_sort_parallel, prefix_sums_dc_parallel — unfixable per "no sequentializing" rule
- **f64 arithmetic** (1 hole): etsp — outside Verus's domain for floating point
- **Closure spec gap** (1 hole): prefix_sums_dc — same root cause as usize overflow
...
3. **`etsp` spec is weak** — only ensures length preservation. No structural validity (cycle, edge connectivity) is expressed. Acceptable given f64 limitations, but noted.

4. **No `ETSPMtPer` module exists.** APAS discusses eTSP with Span Θ(lg² n), implying parallel recursive calls. Only the sequential variant is implemented.
...
8. **Test coverage is comprehensive** — all modules have corresponding test files with meaningful assertions.
