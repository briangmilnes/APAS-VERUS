...
pub mod AVLTreeSeqMtPer {
    ...
    proof fn lemma_height_le_size<T: StTInMtT>(link: &Link<T>)
        requires spec_avltreeseqmtper_wf(*link),
    ...
    pub trait AVLTreeSeqMtPerLinkSpec<T: StTInMtT>: Sized {
    ...
    fn mk<T: StTInMtT>(value: T, left: Link<T>, right: Link<T>) -> (node: Arc<Node<T>>)
        ...
        ensures
            ...
            spec_avltreeseqmtper_wf(left) && spec_avltreeseqmtper_wf(right) ==> spec_avltreeseqmtper_wf(Some(node)),
            node.left == left,
            node.right == right,
    {
    ...
    /// - Alg Analysis: Code review (Claude Opus 4.6): Work O(1), Span O(1)
    impl<T: StTInMtT> AVLTreeSeqMtPerNodeFns<T> for Arc<Node<T>> {
    ...
    fn compare_trees(&self, other: &Self) -> (equal: bool)
    {
        ...
        while i < sa
            invariant
                ...
                spec_avltreeseqmtper_wf(*self),
                spec_avltreeseqmtper_wf(*other),
                obeys_feq_full::<T>(),
                0 <= i <= sa,
