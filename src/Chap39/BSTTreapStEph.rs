...
//	Section 9c. impls
//	Section 10c. iterators — BSTTreapStEph
...
pub mod BSTTreapStEph {
    ...
    #[cfg(verus_keep_ghost)]
    use crate::Chap39::BSTTreapSpecsAndLemmas::BSTTreapSpecsAndLemmas::lemma_split_result_subset as lemma_split_result_subset_st;
    #[cfg(verus_keep_ghost)]
    use crate::Chap39::BSTTreapSpecsAndLemmas::BSTTreapSpecsAndLemmas::lemma_union_part_subset as lemma_union_part_subset_st;
    #[cfg(verus_keep_ghost)]
    use crate::Chap39::BSTTreapSpecsAndLemmas::BSTTreapSpecsAndLemmas::lemma_halves_cross_ordered as lemma_halves_cross_ordered_st;
...
// Veracity: UNNEEDED proof block                             if Self::spec_bst_link(&link) {
// Veracity: UNNEEDED proof block                                 Self::lemma_bst_decompose(&link);
...
// Veracity: UNNEEDED proof block                                         // contradicting key < root_key.
                                        // Veracity: NEEDED assert
                                        // Veracity: NEEDED assert
            ...
            match expose_to_parts_st(left) {
                ...
                Some((ll, lk, lp, lr)) => {
                    ...
                    proof {
                        ...
                        // Explicit subset helps Z3 connect lrv membership to left@ ordering.
            ...
            | Some((left, root_key, root_pri, right)) => {
                ...
                match key.cmp(&root_key) {
                    | Less => {
                        ...
                        // Veracity: NEEDED proof block
                        proof {
                            lemma_split_result_subset_st(llv, lrv, lv, key@);
                    ...
                    | Greater => {
                        ...
                        // Veracity: NEEDED proof block
                        proof {
                            lemma_split_result_subset_st(rlv, rrv, rv, key@);
        ...
        decreases left@.len() + right@.len(),
    {
        ...
        match expose_to_parts_st(right) {
            ...
            | Some((r_left, r_key, rp, r_right)) => {
                ...
                // Veracity: NEEDED proof block
                proof {
                    // split ensures: slv.union(srv) =~= lv.remove(rkv), and rkv ∉ lv.
                    lemma_split_result_subset_st(slv, srv, lv, rkv);
                    // Veracity: NEEDED proof block
        ...
        decreases a@.len(),
    {
        match expose_to_parts_st(a) {
            ...
            | Some((al, ak, ap, ar)) => {
                ...
                // Veracity: NEEDED proof block
                proof {
                    lemma_split_result_subset_st(blv, brv, bv, akv);
        ...
        match expose_to_parts_st(a) {
            ...
            | Some((al, ak, ap, ar)) => {
                ...
                proof {
                    ...
                    lemma_halves_cross_ordered_st(lrv, rrv, ak);
            ...
            | Some((al, ak, ap, ar)) => {
                ...
                proof {
                    ...
                    assert forall|t: T| #[trigger] arv.contains(t@) implies t.cmp_spec(&ak) == Greater by {
                        // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                         assert(ar@.contains(t@));
                    };
                    // Veracity: NEEDED assert
                    // Veracity: NEEDED assert (speed hint)
                    ...
                    assert forall|x| #[trigger] av.difference(bv).contains(x) <==>
                        ...
                        if av.difference(bv).contains(x) {
                            ...
                            if x == akv {
                                ...
                                // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                                 assert(lrv.contains(x));
                            } else {
                                // Veracity: NEEDED assert
                                // Veracity: NEEDED assert (speed hint)
                            ...
                            if lrv.contains(x) {
                                ...
                                // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                                 assert(arv.contains(x) && !brv.contains(x));
                                // Veracity: NEEDED assert
                                // Veracity: NEEDED assert (speed hint)
                                assert(av.contains(x));
                                if bv.contains(x) {
                                    ...
                                    if blv.contains(x) {
                                        ...
                                        view_ord_consistent_st::<T>();
                                        // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                                         assert(t_ar.cmp_spec(&t_bl) == Equal);
                    ...
                    // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                     assert(av.difference(bv) =~= lrv.union(rrv).union(
// Veracity: UNNEEDED assert                         if !found { Set::<<T as View>::V>::empty().insert(akv) }
// Veracity: UNNEEDED assert                         else { Set::<<T as View>::V>::empty() }));
                    // Veracity: NEEDED assert
                    // Veracity: NEEDED assert (speed hint)
                    ...
                    assert forall|t: T| #[trigger] lrv.contains(t@) implies t.cmp_spec(&ak) == Less by {
                        // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                         assert(alv.contains(t@));
                    };
                    // Veracity: NEEDED assert
                    // Veracity: NEEDED assert (speed hint)
                    assert forall|t: T| #[trigger] rrv.contains(t@) implies t.cmp_spec(&ak) == Greater by {
                        // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                         assert(arv.contains(t@));
                    };
                    // Veracity: NEEDED assert
                    // Veracity: NEEDED assert (speed hint)
                    ...
                    assert(alv.len() + arv.len() < av.len());
                    // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                     assert(left_res@.len() + right_res@.len() < usize::MAX as nat);
                }
                if found {
                    join_pair_inner_st(left_res, right_res)
                } else {
        ...
        match expose_to_parts_st(tree) {
            ...
            | Some((left, key, ap, right)) => {
                ...
                proof {
                    ...
                    // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                     assert(!right_filtered@.contains(kv));
                    // Veracity: NEEDED assert
                    // Veracity: NEEDED assert (speed hint)
                    ...
                    lemma_halves_cross_ordered_st(left_filtered@, right_filtered@, key);
                ...
                if keep {
                    ...
                    proof {
                        ...
                        let rf = right_filtered@;
                        // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                         assert forall|v: T::V| #[trigger]
// Veracity: UNNEEDED assert                             lf.union(rf).contains(v) implies spec_pred(v) by {
// Veracity: UNNEEDED assert                         };
    ...
    /// Snapshot iterator over BSTTreapStEph — collects elements via in_order traversal,
    /// then yields owned T values from the captured Vec.
    #[verifier::reject_recursive_types(T)]
    pub struct BSTTreapStEphIter<T: StT + Ord + IsLtTransitive> {
        pub inner: IntoIter<T>,
    }

    impl<T: StT + Ord + IsLtTransitive> View for BSTTreapStEphIter<T> {
        type V = (int, Seq<T>);
        open spec fn view(&self) -> (int, Seq<T>) { self.inner@ }
    }

    pub open spec fn iter_invariant_bsttreapsteph<T: StT + Ord + IsLtTransitive>(it: &BSTTreapStEphIter<T>) -> bool {
        0 <= it@.0 <= it@.1.len()
    }

    impl<T: StT + Ord + IsLtTransitive> std::iter::Iterator for BSTTreapStEphIter<T> {
        type Item = T;
    ...
    /// Ghost iterator for for-loop support over BSTTreapStEphIter.
    #[verifier::reject_recursive_types(T)]
    pub struct BSTTreapStEphGhostIterator<T: StT + Ord + IsLtTransitive> {
        pub pos: int,
        pub elements: Seq<T>,
    }

    impl<T: StT + Ord + IsLtTransitive> View for BSTTreapStEphGhostIterator<T> {
        type V = Seq<T>;
        open spec fn view(&self) -> Seq<T> { self.elements.take(self.pos) }
    }

    impl<T: StT + Ord + IsLtTransitive> vstd::pervasive::ForLoopGhostIteratorNew for BSTTreapStEphIter<T> {
        type GhostIter = BSTTreapStEphGhostIterator<T>;
        open spec fn ghost_iter(&self) -> BSTTreapStEphGhostIterator<T> {
            BSTTreapStEphGhostIterator { pos: self@.0, elements: self@.1 }
        }
    }

    impl<T: StT + Ord + IsLtTransitive> vstd::pervasive::ForLoopGhostIterator for BSTTreapStEphGhostIterator<T> {
        type ExecIter = BSTTreapStEphIter<T>;
        type Item = T;
        type Decrease = int;

        open spec fn exec_invariant(&self, exec_iter: &BSTTreapStEphIter<T>) -> bool {
        ...
        open spec fn ghost_advance(&self, _exec_iter: &BSTTreapStEphIter<T>) -> BSTTreapStEphGhostIterator<T> {
            Self { pos: self.pos + 1, ..*self }
        }
    }

    impl<'a, T: StT + Ord + IsLtTransitive> std::iter::IntoIterator for &'a BSTTreapStEph<T> {
        type Item = T;
        type IntoIter = BSTTreapStEphIter<T>;
                ...
                iter_invariant_bsttreapsteph(&it),
        {
            ...
            BSTTreapStEphIter { inner: in_ord.seq.into_iter() }
        }
    }
    ...
    impl<T: StT + Ord + IsLtTransitive + fmt::Debug> fmt::Debug for BSTTreapStEphIter<T> {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            write!(f, "BSTTreapStEphIter")
        }
    }

    impl<T: StT + Ord + IsLtTransitive + fmt::Display> fmt::Display for BSTTreapStEphIter<T> {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            write!(f, "BSTTreapStEphIter")
        }
    }

    impl<T: StT + Ord + IsLtTransitive + fmt::Debug> fmt::Debug for BSTTreapStEphGhostIterator<T> {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            write!(f, "BSTTreapStEphGhostIterator")
        }
    }

    impl<T: StT + Ord + IsLtTransitive + fmt::Display> fmt::Display for BSTTreapStEphGhostIterator<T> {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            write!(f, "BSTTreapStEphGhostIterator")
        }
    }
