...
pub mod BFSMtPer {
    ...
    pub trait BFSMtPerTrait {
        ...
        /// - Alg Analysis: Code review (Claude Opus 4.6): Work O(n + m), Span O(n + m) — DIFFERS: sequential layer-by-layer BFS, not parallel Alg 54.4
        ...
        /// - Alg Analysis: Code review (Claude Opus 4.6): Work O(n + m), Span O(n + m) — DIFFERS: sequential layer-by-layer BFS tree, span = work
    ...
    fn process_frontier_parallel(
        ...
        requires
            ...
            forall|j: int| 0 <= j < frontier@.len() ==>
                #[trigger] frontier@[j] < graph.spec_len(),
        ensures
            forall|j: int| 0 <= j < traversal.0@.len() ==>
                #[trigger] traversal.0@[j] < graph.spec_len(),
        ...
        decreases frontier@.len()
    {
            ...
            while i < num_neighbors
                invariant
                    ...
                    *neighbors == graph.spec_index(u as int),
                    ...
                    forall|j: int| 0 <= j < next_verts@.len() ==>
                        #[trigger] next_verts@[j] < n,
        ...
        while k < right_verts.len()
            invariant
                0 <= k <= right_verts@.len(),
                forall|j: int| 0 <= j < all_verts@.len() ==>
                    (#[trigger] all_verts@[j] as int) < n_spec,
                forall|j: int| 0 <= j < right_verts@.len() ==>
                    (#[trigger] right_verts@[j] as int) < n_spec,
                    ...
                    parents.spec_index(order@[j] as int) != NO_PARENT,
        {
            ...
            while k < tree_updates.len()
                invariant
                    ...
                    forall|j: int| 0 <= j < next_layer@.len() ==>
