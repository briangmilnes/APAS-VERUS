...
- **StackStEph fully verified (0 external_body):** Removed all 8 `external_body` annotations (including duplicate on `new`). All functions (`new`, `push`, `pop`, `peek`, `is_empty`, `size`, `default`) now verified — they are thin wrappers around Vec operations whose specs match directly.
- `DijkstraStEphI64` / `DijkstraStEphFloat`: trait inside `verus!`, impl cfg-gated (pending Chap45/Chap23 fix).
- **Total external_body:** 8 (down from 16).
...
StackStEph now has `ensures` on all functions (view-level LIFO semantics). Dijkstra functions still lack specs. Spec fidelity: **partial** (StackStEph) / **N/A** (Dijkstra).
...
StackStEph fully verified with `ensures` on all functions. No loops or iterators in StackStEph (all delegated to Vec). Dijkstra modules still `external_body`. No PTTs needed until Dijkstra is verusified.
...
| # | Action | Priority |
|---|--------|----------|
| 1 | Uncomment DijkstraStEphI64/DijkstraStEphFloat in lib.rs (pending Chap45/Chap23 fix) | High |
| 2 | Move PQEntry `Clone`/`PartialEq`/`Eq` to `PartialEqSpecImpl` pattern | Medium |
| 3 | Add `ensures` to `Ord`/`PartialOrd` impls | Medium |
| 4 | Replace `HashMap` with verified table for visited set in Dijkstra | Medium |
| 5 | Verify runtime tests run (Dijkstra modules commented out may block) | Medium |
| 6 | Consider removing unused StackStEph or documenting its intended use | Low |
| 7 | Implement Exercise 57.1 (decreaseKey variant) | Low |
| 8 | Add Example 57.3 as runtime test | Low |
