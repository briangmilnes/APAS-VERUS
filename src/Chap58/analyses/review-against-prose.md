...
**Partial verusification (Int only).** The `BellmanFordStEphIntTrait` trait definition is now inside `verus!`, so Verus type-checks the trait's function signature. However, the trait has **no `requires`/`ensures` specs** — the signature is:
...
**lib.rs gate change:** Chap58 modules are no longer behind `#[cfg(not(verus_keep_ghost))]` in `lib.rs`. The chapter is now compiled by Verus (the `verus!` block in BellmanFordStEphInt is processed during verification). BellmanFordStEphFloat still has no `verus!` block but is no longer gated out of Verus compilation.
...
Neither file has the standard Table of Contents block or section headers. BellmanFordStEphInt now has a `verus!` block but it contains only the trait definition — not enough structure to warrant the full TOC.

### In/Out Table

| # | File | Clone | PartialEq/Eq | Default | Drop | Iterator | Debug | Display | Macro | Other |
|---|------|:-----:|:------------:|:-------:|:----:|:--------:|:-----:|:-------:|:-----:|-------|
| 1 | BellmanFordStEphInt.rs | - | - | - | - | - | - | - | - | Trait ✅ in |
| 2 | BellmanFordStEphFloat.rs | - | - | - | - | - | - | - | - | Trait ❌ out |

**Notes:** BellmanFordStEphInt now has a `verus!` block containing the `BellmanFordStEphIntTrait` trait definition. The impl functions (`bellman_ford`, `reconstruct_predecessors`) are outside `verus!` with `#[cfg(not(verus_keep_ghost))]`. BellmanFordStEphFloat has no `verus!` block — the trait and all functions are plain Rust outside Verus verification.
...
Zero proof holes — vacuously true for BellmanFordStEphFloat (no `verus!` block). For BellmanFordStEphInt, the `verus!` block contains only the trait signature with no proof obligations.
...
**All 4 functions have no specs.** The trait signature inside `verus!` in BellmanFordStEphInt has no `requires`/`ensures`, so it counts as `none`.

## Overall Assessment

Chapter 58 is a **partially verusified but unspecified** implementation of Bellman-Ford's algorithm. The Int variant has its trait definition inside `verus!` and its impl functions cfg-gated behind `#[cfg(not(verus_keep_ghost))]`. The Float variant remains entirely outside `verus!`.
...
5. **Partial verusification (Int)** — BellmanFordStEphInt trait is inside `verus!`, establishing a foundation for future spec work. The module is no longer gated out of Verus compilation in `lib.rs`.
...
7. **Float variant not verusified** — BellmanFordStEphFloat has no `verus!` block at all. The trait and all functions are plain Rust.
...
4. Add `requires`/`ensures` specs to the trait methods in both Int and Float variants.
5. Add TOC headers and restructure files per project standard.
6. Replace `HashMap` with verified table or sequence for distance storage.
7. Implement a parallel `BellmanFordMtEph` variant to achieve the APAS Span of O(n lg n).
8. Add Example 58.4 as a runtime test to validate the algorithm trace.
