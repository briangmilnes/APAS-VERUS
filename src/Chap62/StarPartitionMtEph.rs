    ...
    verus!
{
    ...
    // Arc clone: uses vstd's `Arc::clone` specification (vstdplus/smart_ptrs.rs).
    ...
    fn hash_coin_flips_mt<V: StT + MtT + Hash + Ord + ClonePreservesView + 'static>(
        ...
        ensures
            forall|j: int| start as int <= j < end as int ==>
                #[trigger] key_view(flips@).contains_key(vertices@[j]@),
        decreases end - start,
    {
        let size = end - start;
        if size == 0 {
        ...
        let f1 = move || -> (r: HashMap<V, bool>)
            ...
            ensures
                forall|j: int| start as int <= j < mid as int ==>
                    #[trigger] key_view(r@).contains_key(v1@[j]@),
        {
        ...
        let f2 = move || -> (r: HashMap<V, bool>)
            ...
            ensures
                forall|j: int| mid as int <= j < end as int ==>
                    #[trigger] key_view(r@).contains_key(v2@[j]@),
        {
        ...
        // Merge right entries into merged by delegated iteration over the
        ...
        for kv in it: rit
            invariant
                ...
                forall|j: int| start as int <= j < mid as int ==>
                    #[trigger] key_view(merged@).contains_key(verts_view[j]@),
                // merged contains all iterated entries from right.
                forall|idx: int| 0 <= idx < it.index() ==>
                    #[trigger] key_view(merged@).contains_key(it_seq[idx].0@),
                // iter covers all right keys.
                forall|k: V| #[trigger] right@.contains_key(k) ==>
                    exists|pair: (V, bool)| #[trigger] it_seq.contains(pair) && pair.0 == k,
                // right covers [mid, end).
                forall|j: int| mid as int <= j < end as int ==>
                    #[trigger] key_view(right@).contains_key(verts_view[j]@),
        {
            let (k, v) = kv;
            let ghost pre_merged = key_view(merged@);
            merged.insert(k.clone_view(), *v);
            // Veracity: NEEDED proof block
            // Veracity: NEEDED proof block (speed hint)
            proof {
                // The iterator yields it_seq[it.index()], so the inserted key has its view.
                assert(it_seq[it.index()].0 == *k);
            }
        }
        ...
        // Veracity: NEEDED proof block
        proof {
            // The for loop ends with every entry of it_seq consumed.
            // Veracity: NEEDED assert
            // Veracity: NEEDED assert
            assert forall|j: int| start as int <= j < end as int implies
                #[trigger] key_view(merged@).contains_key(verts_view[j]@) by {
                if j < mid as int {
                    ...
                    // Veracity: NEEDED assert
                    // Veracity: NEEDED assert
                    assert(key_view(right@).contains_key(verts_view[j]@));
                    // key_view: some raw key k of right has the view verts_view[j]@.
                    let k = choose|k: V| #[trigger] right@.contains_key(k) && k@ == verts_view[j]@;
                    // iter ensures: exists pair in it_seq with pair.0 == k.
                    let pair = choose|pair: (V, bool)| #[trigger] it_seq.contains(pair) && pair.0 == k;
    ...
    fn build_th_edges_mt<V: StT + MtT + Hash + Ord + ClonePreservesView + 'static>(
        ...
        vertex_to_index: Arc<HashMap<V, usize>>,
        vertices: Arc<Vec<V>>,
        nv: usize,
        start: usize,
        end: usize,
        ...
        requires
            ...
            forall|k: int| start as int <= k < end as int ==>
                #[trigger] key_view(coin_flips@).contains_key(edges@[k]@.0) &&
                key_view(coin_flips@).contains_key(edges@[k]@.1) &&
                key_view(vertex_to_index@).contains_key(edges@[k]@.0) &&
                key_view(vertex_to_index@).contains_key(edges@[k]@.1),
            forall|j: int| 0 <= j < nv as int ==>
                #[trigger] key_view(vertex_to_index@).contains_key(vertices@[j]@) &&
                key_view(vertex_to_index@)[vertices@[j]@] as usize == j,
            forall|v_view: V::V| #[trigger] key_view(vertex_to_index@).contains_key(v_view) ==>
                exists|j: int| 0 <= j < nv as int && #[trigger] vertices@[j]@ == v_view,
        ensures
            forall|s: int| 0 <= s < th_edges@.len() ==>
                #[trigger] spec_valid_th_entry(th_edges@[s], nv as nat, key_view(coin_flips@), vertices@, key_view(vertex_to_index@)),
        decreases end - start,
    {
        let size = end - start;
        if size == 0 {
        ...
        if size == 1 {
            ...
            proof {
                ...
                // Veracity: NEEDED assert
// Veracity: UNNEEDED assert                 assert(key_view(coin_flips@).contains_key(u@));
            }
        ...
        let mid = start + size / 2;
        let ghost cf_view = key_view(coin_flips@);
        let ghost vt_view = vertices@;
        let ghost vi_view = key_view(vertex_to_index@);
        let e1 = edges.clone();
        ...
        let f1 = move || -> (r: Vec<(usize, V)>)
            requires
                ...
                forall|k: int| start as int <= k < mid as int ==>
                    #[trigger] key_view(cf1@).contains_key(e1@[k]@.0) && key_view(cf1@).contains_key(e1@[k]@.1) &&
                    key_view(vi1@).contains_key(e1@[k]@.0) && key_view(vi1@).contains_key(e1@[k]@.1),
                forall|j: int| 0 <= j < nv as int ==>
                    #[trigger] key_view(vi1@).contains_key(vt1@[j]@) && key_view(vi1@)[vt1@[j]@] as usize == j,
                forall|v_view: V::V| #[trigger] key_view(vi1@).contains_key(v_view) ==>
                    exists|j: int| 0 <= j < nv as int && #[trigger] vt1@[j]@ == v_view,
            ensures
                forall|s: int| 0 <= s < r@.len() ==>
                    #[trigger] spec_valid_th_entry(r@[s], nv as nat, key_view(cf1@), vt1@, key_view(vi1@)),
        {
        ...
        let f2 = move || -> (r: Vec<(usize, V)>)
            requires
                ...
                forall|k: int| mid as int <= k < end as int ==>
                    ...
                    key_view(vi2@).contains_key(e2@[k]@.0) && key_view(vi2@).contains_key(e2@[k]@.1),
                forall|j: int| 0 <= j < nv as int ==>
                    #[trigger] key_view(vi2@).contains_key(vt2@[j]@) && key_view(vi2@)[vt2@[j]@] as usize == j,
                forall|v_view: V::V| #[trigger] key_view(vi2@).contains_key(v_view) ==>
                    exists|j: int| 0 <= j < nv as int && #[trigger] vt2@[j]@ == v_view,
            ensures
                forall|s: int| 0 <= s < r@.len() ==>
                    #[trigger] spec_valid_th_entry(r@[s], nv as nat, key_view(cf2@), vt2@, key_view(vi2@)),
        {
    ...
    fn build_vertex_to_index_mt<V: StT + MtT + Hash + Ord + ClonePreservesView + 'static>(
        ...
        ensures
            forall|j: int| start as int <= j < end as int ==>
                #[trigger] key_view(vi@).contains_key(vertices@[j]@) &&
                key_view(vi@)[vertices@[j]@] as usize == j,
            forall|v_view: V::V| #[trigger] key_view(vi@).contains_key(v_view) ==>
        ...
        decreases end - start,
    {
        ...
        let f1 = move || -> (r: HashMap<V, usize>)
            ...
            ensures
                forall|j: int| start as int <= j < mid as int ==>
                    #[trigger] key_view(r@).contains_key(v1@[j]@) && key_view(r@)[v1@[j]@] as usize == j,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    exists|j: int| start as int <= j < mid as int && #[trigger] v1@[j]@ == v_view,
        {
            build_vertex_to_index_mt(v1, start, mid)
        };

        let f2 = move || -> (r: HashMap<V, usize>)
            requires
                mid <= end, end <= v2@.len(),
                v2@.no_duplicates(),
                valid_key_type_Edge::<V>(),
            ensures
                forall|j: int| mid as int <= j < end as int ==>
                    #[trigger] key_view(r@).contains_key(v2@[j]@) && key_view(r@)[v2@[j]@] as usize == j,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
...
// Veracity: NEEDED proof block
        ...
        while j < end
            invariant
                ...
                forall|j2: int| start as int <= j2 < mid as int ==>
                    #[trigger] key_view(merged@).contains_key(verts_view[j2]@) &&
                    key_view(merged@)[verts_view[j2]@] as usize == j2,
                ...
                forall|v_view: V::V| #[trigger] key_view(merged@).contains_key(v_view) ==>
            ...
            decreases end - j,
        {
            ...
            proof {
                ...
                assert forall|j2: int| start as int <= j2 < mid as int implies
                    #[trigger] key_view(merged@).contains_key(verts_view[j2]@) &&
                    key_view(merged@)[verts_view[j2]@] as usize == j2 by {
                ...
                // Veracity: NEEDED assert
                // Veracity: NEEDED assert
                assert forall|v_view: V::V| #[trigger] key_view(merged@).contains_key(v_view) implies
                    dom_witnesses.contains_key(v_view) &&
                    start as int <= dom_witnesses[v_view] &&
    ...
    fn build_satellite_map_mt<V: StT + MtT + Hash + Ord + ClonePreservesView + 'static>(
        ...
        ensures
            ...
            forall|v_view: V::V| #[trigger] key_view(sm@).contains_key(v_view) ==>
            ...
            forall|v_view: V::V| #[trigger] key_view(sm@).contains_key(v_view) ==>
                key_view(coin_flips@).contains_key(key_view(sm@)[v_view]@) &&
                key_view(coin_flips@)[key_view(sm@)[v_view]@] &&
                key_view(vertex_to_index@).contains_key(key_view(sm@)[v_view]@) &&
                (key_view(vertex_to_index@)[key_view(sm@)[v_view]@] as usize) < nv,
            // Keys are tails vertices (not heads).
            forall|v_view: V::V| #[trigger] key_view(sm@).contains_key(v_view) ==>
                key_view(coin_flips@).contains_key(v_view) &&
                !key_view(coin_flips@)[v_view],
        decreases end - start,
    // Veracity: NEEDED proof block
    {
        let size = end - start;
        ...
        if size == 1 {
            ...
            proof {
                // Veracity: NEEDED assert
                // Veracity: NEEDED assert
                assert(spec_valid_th_entry(th_edges@[start as int], nv as nat, key_view(coin_flips@), vertices@, key_view(vertex_to_index@)));
            }
            if (idx as usize) < nv {
        ...
        let f1 = move || -> (r: HashMap<V, V>)
            requires
                ...
                forall|s: int| 0 <= s < the1@.len() ==>
                    #[trigger] spec_valid_th_entry(the1@[s], nv as nat, key_view(cf1@), v1@, key_view(vi1@)),
            ensures
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    exists|s: int| start as int <= s < mid as int &&
                        #[trigger] v1@[the1@[s].0 as int]@ == v_view,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    key_view(cf1@).contains_key(key_view(r@)[v_view]@) && key_view(cf1@)[key_view(r@)[v_view]@] &&
                    key_view(vi1@).contains_key(key_view(r@)[v_view]@) && (key_view(vi1@)[key_view(r@)[v_view]@] as usize) < nv,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    key_view(cf1@).contains_key(v_view) && !key_view(cf1@)[v_view],
        {
        ...
        let f2 = move || -> (r: HashMap<V, V>)
            requires
                ...
                forall|s: int| 0 <= s < the2@.len() ==>
                    #[trigger] spec_valid_th_entry(the2@[s], nv as nat, key_view(cf2@), v2@, key_view(vi2@)),
            ensures
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    exists|s: int| mid as int <= s < end as int &&
                        #[trigger] v2@[the2@[s].0 as int]@ == v_view,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    key_view(cf2@).contains_key(key_view(r@)[v_view]@) && key_view(cf2@)[key_view(r@)[v_view]@] &&
                    key_view(vi2@).contains_key(key_view(r@)[v_view]@) && (key_view(vi2@)[key_view(r@)[v_view]@] as usize) < nv,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    key_view(cf2@).contains_key(v_view) && !key_view(cf2@)[v_view],
        {
        ...
        while t < end
            invariant
                ...
                forall|v_view: V::V| #[trigger] key_view(merged@).contains_key(v_view) ==>
                    // Veracity: NEEDED proof block
                    key_view(cf@).contains_key(key_view(merged@)[v_view]@) && key_view(cf@)[key_view(merged@)[v_view]@] &&
                    key_view(vi@).contains_key(key_view(merged@)[v_view]@) && (key_view(vi@)[key_view(merged@)[v_view]@] as usize) < nv,
                // Keys are tails.
                forall|v_view: V::V| #[trigger] key_view(merged@).contains_key(v_view) ==>
                    key_view(cf@).contains_key(v_view) && !key_view(cf@)[v_view],
                // th_edges invariant.
                forall|s: int| 0 <= s < the_view.len() ==>
                    #[trigger] spec_valid_th_entry(the_view[s], nv as nat, key_view(cf@), verts_view, key_view(vi@)),
            decreases end - t,
        {
            ...
            proof {
                // Veracity: NEEDED assert
                // Veracity: NEEDED assert
                assert(spec_valid_th_entry(the_view[t as int], nv as nat, key_view(cf@), verts_view, key_view(vi@)));
            }
            if (idx as usize) < nv {
                let ghost pre_merged = key_view(merged@);
                ...
                proof {
                    ...
                    assert forall|v_view: V::V| #[trigger] key_view(merged@).contains_key(v_view) implies
                        key_view(cf@).contains_key(key_view(merged@)[v_view]@) && key_view(cf@)[key_view(merged@)[v_view]@] &&
                        key_view(vi@).contains_key(key_view(merged@)[v_view]@) && (key_view(vi@)[key_view(merged@)[v_view]@] as usize) < nv by {
                        if v_view == tail_v {
                        } else {
                        }
                    };
                    ...
                    // Veracity: NEEDED assert
                    assert forall|v_view: V::V| #[trigger] key_view(merged@).contains_key(v_view) implies
                        key_view(cf@).contains_key(v_view) && !key_view(cf@)[v_view] by {
                        if v_view == tail_v {
                        } else {
                        }
                    };
    ...
    fn build_p_vec_with_inject_mt<V: StT + MtT + Hash + Ord + ClonePreservesView + 'static>(
        ...
        requires
            ...
            forall|j: int| 0 <= j < nv as int ==>
                #[trigger] key_view(coin_flips@).contains_key(vertices@[j]@),
            // satellite_map values are heads with valid vi entries.
            forall|v_view: V::V| #[trigger] key_view(satellite_map@).contains_key(v_view) ==>
                key_view(coin_flips@).contains_key(key_view(satellite_map@)[v_view]@) &&
                key_view(coin_flips@)[key_view(satellite_map@)[v_view]@] &&
                key_view(vertex_to_index@).contains_key(key_view(satellite_map@)[v_view]@) &&
                (key_view(vertex_to_index@)[key_view(satellite_map@)[v_view]@] as usize) < nv,
            // satellite_map keys are tails.
            forall|v_view: V::V| #[trigger] key_view(satellite_map@).contains_key(v_view) ==>
                key_view(coin_flips@).contains_key(v_view) && !key_view(coin_flips@)[v_view],
        ...
        ensures
            ...
            forall|j: int| 0 <= j < p_vec@.len() ==>
                key_view(coin_flips@).contains_key(vertices@[(start as int + j)]@) &&
                (key_view(coin_flips@)[vertices@[(start as int + j)]@] ==>
                 #[trigger] p_vec@[j]@ == vertices@[(start as int + j)]@),
            // Modified entries point to heads.
            forall|j: int| 0 <= j < p_vec@.len() ==>
                p_vec@[j]@ != vertices@[(start as int + j)]@ ==>
                (key_view(coin_flips@).contains_key(#[trigger] p_vec@[j]@) &&
                 key_view(coin_flips@)[p_vec@[j]@]),
            // All entries in vertex_to_index.
            forall|j: int| 0 <= j < p_vec@.len() ==>
                key_view(vertex_to_index@).contains_key(#[trigger] p_vec@[j]@) &&
                (key_view(vertex_to_index@)[p_vec@[j]@] as usize) < nv,
        // Veracity: NEEDED proof block
        decreases end - start,
    {
        let size = end - start;
            ...
            match sm.get(&verts[start]) {
                ...
                None => {
                    ...
                    proof {
                        ...
                        // Veracity: NEEDED assert
                        // Veracity: NEEDED assert
                        assert(key_view(vertex_to_index@).contains_key(sv));
                    }
                },
            }
            return result;
        ...
        let f1 = move || -> (r: Vec<V>)
            requires
                ...
                valid_key_type_Edge::<V>(),
                forall|j: int| 0 <= j < nv as int ==>
                    #[trigger] key_view(cf1@).contains_key(v1@[j]@),
                forall|v_view: V::V| #[trigger] key_view(sm1@).contains_key(v_view) ==>
                    key_view(cf1@).contains_key(key_view(sm1@)[v_view]@) && key_view(cf1@)[key_view(sm1@)[v_view]@] &&
                    key_view(vi1@).contains_key(key_view(sm1@)[v_view]@) && (key_view(vi1@)[key_view(sm1@)[v_view]@] as usize) < nv,
                forall|v_view: V::V| #[trigger] key_view(sm1@).contains_key(v_view) ==>
                    key_view(cf1@).contains_key(v_view) && !key_view(cf1@)[v_view],
                forall|j: int| 0 <= j < nv as int ==>
                    #[trigger] key_view(vi1@).contains_key(v1@[j]@) && key_view(vi1@)[v1@[j]@] as usize == j,
                forall|v_view: V::V| #[trigger] key_view(vi1@).contains_key(v_view) ==>
                    exists|j: int| 0 <= j < nv as int && #[trigger] v1@[j]@ == v_view,
            ensures
                r@.len() == (mid - start) as int,
                forall|j: int| 0 <= j < r@.len() ==>
                    key_view(cf1@).contains_key(v1@[(start as int + j)]@) &&
                    (key_view(cf1@)[v1@[(start as int + j)]@] ==>
                     #[trigger] r@[j]@ == v1@[(start as int + j)]@),
                forall|j: int| 0 <= j < r@.len() ==>
                    r@[j]@ != v1@[(start as int + j)]@ ==>
                    (key_view(cf1@).contains_key(#[trigger] r@[j]@) && key_view(cf1@)[r@[j]@]),
                forall|j: int| 0 <= j < r@.len() ==>
                    key_view(vi1@).contains_key(#[trigger] r@[j]@) && (key_view(vi1@)[r@[j]@] as usize) < nv,
        {
        ...
        let f2 = move || -> (r: Vec<V>)
            requires
                ...
                valid_key_type_Edge::<V>(),
                forall|j: int| 0 <= j < nv as int ==>
                    #[trigger] key_view(cf2@).contains_key(v2@[j]@),
                forall|v_view: V::V| #[trigger] key_view(sm2@).contains_key(v_view) ==>
                    key_view(cf2@).contains_key(key_view(sm2@)[v_view]@) && key_view(cf2@)[key_view(sm2@)[v_view]@] &&
                    key_view(vi2@).contains_key(key_view(sm2@)[v_view]@) && (key_view(vi2@)[key_view(sm2@)[v_view]@] as usize) < nv,
                forall|v_view: V::V| #[trigger] key_view(sm2@).contains_key(v_view) ==>
                    key_view(cf2@).contains_key(v_view) && !key_view(cf2@)[v_view],
                forall|j: int| 0 <= j < nv as int ==>
                    #[trigger] key_view(vi2@).contains_key(v2@[j]@) && key_view(vi2@)[v2@[j]@] as usize == j,
                forall|v_view: V::V| #[trigger] key_view(vi2@).contains_key(v_view) ==>
                    exists|j: int| 0 <= j < nv as int && #[trigger] v2@[j]@ == v_view,
            ensures
                r@.len() == (end - mid) as int,
                forall|j: int| 0 <= j < r@.len() ==>
                    key_view(cf2@).contains_key(v2@[(mid as int + j)]@) &&
                    (key_view(cf2@)[v2@[(mid as int + j)]@] ==>
                     #[trigger] r@[j]@ == v2@[(mid as int + j)]@),
                forall|j: int| 0 <= j < r@.len() ==>
                    r@[j]@ != v2@[(mid as int + j)]@ ==>
                    (key_view(cf2@).contains_key(#[trigger] r@[j]@) && key_view(cf2@)[r@[j]@]),
                forall|j: int| 0 <= j < r@.len() ==>
                    key_view(vi2@).contains_key(#[trigger] r@[j]@) && (key_view(vi2@)[r@[j]@] as usize) < nv,
        {
        ...
        while i < right.len()
            invariant
                ...
                forall|j: int| 0 <= j < right@.len() ==>
                    key_view(coin_flips@).contains_key(verts_view[(mid as int + j)]@) &&
                    (key_view(coin_flips@)[verts_view[(mid as int + j)]@] ==>
                     // Veracity: NEEDED proof block
                     #[trigger] right@[j]@ == verts_view[(mid as int + j)]@),
                forall|j: int| 0 <= j < right@.len() ==>
                    right@[j]@ != verts_view[(mid as int + j)]@ ==>
                    (key_view(coin_flips@).contains_key(#[trigger] right@[j]@) && key_view(coin_flips@)[right@[j]@]),
                forall|j: int| 0 <= j < right@.len() ==>
                    key_view(vertex_to_index@).contains_key(#[trigger] right@[j]@) &&
                    (key_view(vertex_to_index@)[right@[j]@] as usize) < nv,
        ...
        proof {
            ...
            // Veracity: NEEDED assert
            assert forall|j: int| 0 <= j < result@.len() implies
                key_view(coin_flips@).contains_key(verts_view[(start as int + j)]@) &&
                (key_view(coin_flips@)[verts_view[(start as int + j)]@] ==>
                 #[trigger] result@[j]@ == verts_view[(start as int + j)]@) by {
                if j < left_len {
            ...
            assert forall|j: int| 0 <= j < result@.len() &&
                ...
                key_view(coin_flips@).contains_key(#[trigger] result@[j]@) && key_view(coin_flips@)[result@[j]@] by {
                if j < left_len {
                } else {
                    let rj = j - left_len;
                    // Veracity: NEEDED assert
            ...
            // Veracity: NEEDED assert
            assert forall|j: int| 0 <= j < result@.len() implies
                key_view(vertex_to_index@).contains_key(#[trigger] result@[j]@) &&
                (key_view(vertex_to_index@)[result@[j]@] as usize) < nv by {
                if j < left_len {
                } else {
                    let rj = j - left_len;
                    // Veracity: NEEDED assert
    ...
    fn build_partition_map_mt<V: StT + MtT + Hash + Ord + ClonePreservesView + 'static>(
        ...
        ensures
            forall|j: int| start as int <= j < end as int ==>
                #[trigger] key_view(pm@).contains_key(vertices@[j]@),
            forall|j: int| start as int <= j < end as int ==>
                key_view(pm@).contains_key(vertices@[j]@) ==>
                key_view(pm@)[vertices@[j]@]@ == #[trigger] p_vec@[j]@,
            forall|v_view: V::V| #[trigger] key_view(pm@).contains_key(v_view) ==>
                exists|j: int| start as int <= j < end as int && #[trigger] vertices@[j]@ == v_view,
        decreases end - start,
    {
        let size = end - start;
        ...
        let f1 = move || -> (r: HashMap<V, V>)
            ...
            ensures
                forall|j: int| start as int <= j < mid as int ==>
                    #[trigger] key_view(r@).contains_key(v1@[j]@),
                forall|j: int| start as int <= j < mid as int ==>
                    key_view(r@).contains_key(v1@[j]@) ==> key_view(r@)[v1@[j]@]@ == #[trigger] p1@[j]@,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    exists|j: int| start as int <= j < mid as int && #[trigger] v1@[j]@ == v_view,
        {
        ...
        let f2 = move || -> (r: HashMap<V, V>)
            ...
            ensures
                ...
                forall|j: int| mid as int <= j < end as int ==>
                    // Veracity: NEEDED proof block
                    key_view(r@).contains_key(v2@[j]@) ==> key_view(r@)[v2@[j]@]@ == #[trigger] p2@[j]@,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
                    exists|j: int| mid as int <= j < end as int && #[trigger] v2@[j]@ == v_view,
        {
        ...
        while j < end
            invariant
                ...
                forall|j2: int| start as int <= j2 < mid as int ==>
                    key_view(merged@).contains_key(verts_view[j2]@) ==>
                    key_view(merged@)[verts_view[j2]@]@ == #[trigger] pv_view[j2]@,
            ...
            decreases end - j,
        {
            ...
            proof {
                ...
                assert forall|j2: int| start as int <= j2 < mid as int &&
                    key_view(merged@).contains_key(verts_view[j2]@) implies
                    key_view(merged@)[verts_view[j2]@]@ == #[trigger] pv_view[j2]@ by {
                    let ghost j2v = verts_view[j2]@;
                    if j2v != jv {
                    } else {
                    }
...
// Veracity: NEEDED proof block
    ...
    // Veracity: NEEDED proof block
    {
        ...
        let vertex_to_index = build_vertex_to_index_mt(vertices_arc.clone(), 0, nv);
        ...
        proof {
            ...
            assert forall|v_view: V::V| #[trigger] graph@.V.contains(v_view) implies
                key_view(vertex_to_index@).contains_key(v_view) by {
                let k = vertices_vec@.map(|_i: int, t: V| t@).index_of(v_view);
            };
        }

        // Wrap vertex_to_index in Arc for parallel operations.
        let vtx_to_idx_arc: Arc<HashMap<V, usize>> = Arc::new(vertex_to_index);
        ...
        let coin_flips_arc: Arc<HashMap<V, bool>> = Arc::new(coin_flips_owned);
        ...
        proof {
            ...
            assert forall|k: int| 0 <= k < ne as int implies
                #[trigger] key_view(coin_flips@).contains_key(edge_vec@[k]@.0) &&
                key_view(coin_flips@).contains_key(edge_vec@[k]@.1) &&
                key_view(vertex_to_index@).contains_key(edge_vec@[k]@.0) &&
                key_view(vertex_to_index@).contains_key(edge_vec@[k]@.1) by {
        ...
        let satellite_map = build_satellite_map_mt(
            th_edges_arc, vertices_arc.clone(), nv,
            coin_flips_arc.clone(), vtx_to_idx_arc.clone(), 0, nth,
        );
        let satellite_map_arc: Arc<HashMap<V, V>> = Arc::new(satellite_map);

        // Step 4b: build p_vec with inject (Work O(n), Span O(lg n)).
        let p_vec = build_p_vec_with_inject_mt(
            vertices_arc.clone(), satellite_map_arc,
            coin_flips_arc.clone(), vtx_to_idx_arc.clone(), nv, 0, nv,
        );
        ...
        proof {
            ...
            assert forall|j2: int| 0 <= j2 < nv as int implies
                key_view(coin_flips@).contains_key(vertices_vec@[j2]@) &&
                (key_view(coin_flips@)[vertices_vec@[j2]@] ==>
            ...
            assert forall|j2: int| 0 <= j2 < nv as int &&
                ...
                key_view(coin_flips@).contains_key(#[trigger] p_vec@[j2]@) &&
                 key_view(coin_flips@)[p_vec@[j2]@] by {};
            // All entries in vertex_to_index.
            // Veracity: NEEDED assert
            // Veracity: NEEDED assert
            assert forall|j2: int| 0 <= j2 < nv as int implies
                key_view(vertex_to_index@).contains_key(#[trigger] p_vec@[j2]@) &&
                (key_view(vertex_to_index@)[p_vec@[j2]@] as usize) < nv by {};
        }
        ...
        let f_pm = move || -> (r: HashMap<V, V>)
            ...
            ensures
                forall|j: int| 0 <= j < nv as int ==>
                    #[trigger] key_view(r@).contains_key(va2@[j]@),
                forall|j: int| 0 <= j < nv as int ==>
                    key_view(r@).contains_key(va2@[j]@) ==>
                    key_view(r@)[va2@[j]@]@ == #[trigger] pa2@[j]@,
                forall|v_view: V::V| #[trigger] key_view(r@).contains_key(v_view) ==>
        ...
        proof {
            ...
            // Veracity: NEEDED assert
            assert forall|v_view: V::V| #[trigger] graph@.V.contains(v_view) implies
                key_view(partition_map@).contains_key(v_view) by {
            ...
            // Veracity: NEEDED assert
            // Veracity: NEEDED assert
            assert forall|v_view: V::V| #[trigger] key_view(partition_map@).contains_key(v_view) implies
                centers@.contains(key_view(partition_map@)[v_view]@) by {
                ...
                if h == vertices_vec@[j]@ {
                    ...
                    let ghost q_h = key_view(vertex_to_index@)[h] as usize;
