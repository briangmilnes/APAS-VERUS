...
//! Experiment (r207, agent 3 probe): pe_noinner with iter_adapted moved from OuterTrait to an inherent impl on OuterS. Does moving the method out of the trait make next() verify?
//!
//! RESULT: SUCCEEDS — 3 verified, 0 errors
//! DATE: 2026-09-20
//! VERUS: 0.2026.09.13.671956e
//! LOG: logs/validate-standard-exp_adaptor_pf_inherent.20260920-154841.log
//! LOG: logs/validate-experiment-prophetic_adaptor_pf_inherent.20260920-160421.log (re-run from this path)
//! Context: docs/StandardsUpgrade.md section 2.3; the wrapping (adaptor) iterator standard.

pub mod prophetic_adaptor_pf_inherent {
    ...
    impl<T> OuterS<T> {
        pub fn iter_adapted(&self) -> (it: OuterIter<'_, T>)
            ensures
                IteratorSpec::remaining(&it) == self@.as_ref(),
                it.elts() == self@,
                IteratorSpec::decrease(&it) is Some,
        {
            OuterIter { inner: self.data.seq.iter() }
        }
    }
