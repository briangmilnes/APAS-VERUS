...
//! Experiment: coarse-grained RwLock as a tokenized state machine.
//! Adapted from verus/examples/state_machines/rwlock.rs.
    ...
    tokenized_state_machine!(RwLock {
        ...
        transition!{
            acquire_read_end() {
                ...
                birds_eye let x: T = pre.storage->0;
                add reader += {x};
            }
        }
        ...
        pub fn reader_agrees_storage(&self) -> bool {
            forall |t: T| #[trigger] self.reader.count(t) > 0 ==>
        ...
        pub fn writer_agrees_storage_rev(&self) -> bool {
            self.storage is None ==> self.writer is Some
        }
