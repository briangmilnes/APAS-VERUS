...
verus! {
    ...
    pub fn int_array_mem_for(s: &[int], elt: int) -> (result: bool)
    ...
    pub fn int_vec_mem_for(s: Vec<int>, elt: int) -> (result: bool)
        ensures result == seq_int_mem(s@, elt)
    {
        for i in 0..s.len()
            invariant
...
*/
    ...
    pub fn int_array_find_for(s: &[int], elt: int) -> (result: Option<usize>)
        ensures
            match result {
    ...
    pub fn int_vec_find_for(s: Vec<int>, elt: int) -> (result: Option<usize>)
        ensures
            match result {
                Some(i) => i < s@.len() && s@[i as int] == elt && (forall|j: int| 0 <= j < i ==> s@[j] != elt),
                None => forall|j: int| 0 <= j < s@.len() ==> s@[j] != elt,
    ...
    pub fn int_array_count_up_for(s: &[int], elt: int) -> (count: usize)
        ensures count <= s@.len()
    {
    ...
    pub fn int_vec_count_up_for(s: Vec<int>, elt: int) -> (count: usize)
        ensures count <= s@.len()
    {
        let mut count: usize = 0;
        for i in 0..s.len()
