...
verus! {
    ...
    pub fn i64_array_mem_loop(s: &[i64], elt: i64) -> (result: bool)
...
*/
    ...
    pub fn i64_array_find_loop(s: &[i64], elt: i64) -> (result: Option<usize>)
        ensures
            match result {
    ...
    pub fn i64_vec_find_loop(s: Vec<i64>, elt: i64) -> (result: Option<usize>)
        ensures
            match result {
                Some(i) => i < s@.len() && s@[i as int] == elt && (forall|j: int| 0 <= j < i ==> s@[j] != elt),
                None => forall|j: int| 0 <= j < s@.len() ==> s@[j] != elt,
    ...
    pub fn i64_array_count_up_loop(s: &[i64], elt: i64) -> (count: usize)
        ensures count <= s@.len()
    {
    ...
    pub fn i64_vec_count_up_loop(s: Vec<i64>, elt: i64) -> (count: usize)
        ensures count <= s@.len()
    {
        let mut count: usize = 0;
        let mut i: usize = 0;
