...
verus! {
    ...
    pub fn int_array_mem_loop(s: &[int], elt: int) -> (result: bool)
    ...
    pub fn int_vec_mem_loop(s: Vec<int>, elt: int) -> (result: bool)
        ensures result == seq_int_mem(s@, elt)
    {
        let mut i: usize = 0;
        loop
    ...
    pub fn usize_array_mem_loop(s: &[usize], elt: usize) -> (result: bool)
...
/*
    ...
    pub fn t_array_mem_loop<T: Eq>(s: &[T], elt: T) -> (result: bool)
...
*/
    ...
    pub fn int_array_find_loop(s: &[int], elt: int) -> (result: Option<usize>)
        ensures
            match result {
    ...
    pub fn int_vec_find_loop(s: Vec<int>, elt: int) -> (result: Option<usize>)
        ensures
            match result {
                Some(i) => i < s@.len() && s@[i as int] == elt && (forall|j: int| 0 <= j < i ==> s@[j] != elt),
                None => forall|j: int| 0 <= j < s@.len() ==> s@[j] != elt,
    ...
    pub fn usize_array_find_loop(s: &[usize], elt: usize) -> (result: Option<usize>)
        ensures
            match result {
                Some(i) => i < s@.len() && s@[i as int] == elt && (forall|j: int| 0 <= j < i ==> s@[j] != elt),
                None => forall|j: int| 0 <= j < s@.len() ==> s@[j] != elt,
    ...
    pub fn i64_array_sum_non_negative_up_loop(s: &[i64]) -> (result: bool)
    ...
    pub fn i64_vec_sum_non_negative_up_loop(s: Vec<i64>) -> (result: bool)
        ensures result == seq_i64_sum_non_negative_up(s@)
    {
        let mut sum: i128 = 0;
        let mut i: usize = 0;
    ...
    pub fn i64_array_sum_non_negative_down_loop(s: &[i64]) -> (result: bool)
    ...
    pub fn i64_vec_sum_non_negative_down_loop(s: Vec<i64>) -> (result: bool)
        ensures result == seq_i64_sum_non_negative_down(s@)
    {
        let mut sum: i128 = 0;
        let mut i: usize = s.len();
    ...
    pub fn array_length_up_loop<T>(s: &[T]) -> (length: usize)
    ...
    pub fn vec_length_up_loop<T>(s: Vec<T>) -> (length: usize)
        ensures length == s@.len()
    {
        let mut length: usize = 0;
        let mut i: usize = 0;
    ...
    pub fn int_array_count_up_loop(s: &[int], elt: int) -> (count: usize)
        ensures count <= s@.len()
    {
    ...
    pub fn int_vec_count_up_loop(s: Vec<int>, elt: int) -> (count: usize)
        ensures count <= s@.len()
    {
        let mut count: usize = 0;
        let mut i: usize = 0;
