...
verus! {
    ...
    pub trait CheckedNatWithCheckedViewTrait: Sized + Clone {
        ...
        proof fn lemma_add_commutative_ghost(a: Self, b: Self)
            ensures a.spec_value() + b.spec_value() == b.spec_value() + a.spec_value();
        ...
        proof fn lemma_add_associative_ghost(a: Self, b: Self, c: Self)
            ensures (a.spec_value() + b.spec_value()) + c.spec_value() == a.spec_value() + (b.spec_value() + c.spec_value());

        proof fn lemma_mul_associative_ghost(a: Self, b: Self, c: Self)
            ensures (a.spec_value() * b.spec_value()) * c.spec_value() == a.spec_value() * (b.spec_value() * c.spec_value())
            { vstd::arithmetic::mul::lemma_mul_is_associative(a.spec_value(), b.spec_value(), c.spec_value()); }

        // Distributivity
        proof fn lemma_mul_distributes_over_add_ghost(a: Self, b: Self, c: Self)
            ensures a.spec_value() * (b.spec_value() + c.spec_value()) == a.spec_value() * b.spec_value() + a.spec_value() * c.spec_value();
        ...
        proof fn lemma_sum_monotonic(a: Self, b: Self)
            requires
                ...
                b.spec_value() >= 0,
            ensures
                a.spec_value() <= a.spec_value() + b.spec_value(),
                b.spec_value() <= a.spec_value() + b.spec_value();
        ...
        proof fn lemma_add_normal_if_sum_fits(a: Self, b: Self)
            requires
                a.spec_value() >= 0,
                b.spec_value() >= 0,
                a.spec_value() + b.spec_value() <= Self::spec_max() as int,
            ensures 0 <= a.spec_value() + b.spec_value() <= Self::spec_max() as int;
    }

    // BoundedNat: the view type that knows about overflow state
    pub enum BoundedNat<T> { Normal(T), Overflow(T), }
