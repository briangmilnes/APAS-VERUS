//! HashSetWithViewPlus - extended from vstd's HashSetWithView
//! Adds: clone(), iter(), union(), intersection(), difference()
...
/// `HashSetWithViewPlus` extends vstd's `HashSetWithView` with clone(), iter(), and set operations.
#[verifier::ext_equal]
...
pub broadcast axiom fn axiom_hash_set_with_view_spec_len<Key>(m: &HashSetWithViewPlus<Key>) where
    Key: View + Eq + Hash + Clone,
