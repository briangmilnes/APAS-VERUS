...
pub mod set_axioms {
    ...
    // UNUSED: not called anywhere, testing if needed
    ...
    // UNNEEDED: vstd provides lemma_set_empty_equivalency_len in group_set_lib_default
    // which provides the same functionality but correctly requires s.finite().
    // 
    ...
    // UNUSED: broadcast group not used, broadcast not triggering
    ...
    // {
    // }
    // UNUSED: broadcast group not used anywhere
    // /// Axiom group for additional set axioms
    // pub broadcast group group_set_axioms_plus {
    //     // axiom_set_len_zero_iff_empty,  // UNNEEDED: use vstd's lemma_set_empty_equivalency_len
