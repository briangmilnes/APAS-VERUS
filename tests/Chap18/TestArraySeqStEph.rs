...
//! Runtime tests for ArraySeqStEph (sequential ephemeral array sequence).

use apas_verus::Chap18::ArraySeqStEph::ArraySeqStEph::*;

#[test]
fn test_new() {
    let seq = ArraySeqStEphS::<i32>::new(5, 42);
...
#[test]
fn test_empty() {
    let seq = ArraySeqStEphS::<i32>::empty();
...
#[test]
fn test_singleton() {
    let seq = ArraySeqStEphS::singleton(42);
...
#[test]
fn test_set() {
    let mut seq = ArraySeqStEphS::<i32>::new(3, 0);
    seq.set(1, 99).unwrap();
...
#[test]
fn test_append() {
    let a = ArraySeqStEphS::from_vec(vec![1, 2, 3]);
    let b = ArraySeqStEphS::from_vec(vec![4, 5]);
    let c = ArraySeqStEphS::append(&a, &b);
...
fn test_subseq() {
    ...
    let sub = ArraySeqStEphS::subseq(&seq, 1, 3);
...
fn test_update() {
    ...
    let updated = ArraySeqStEphS::update(&seq, 1, 99);
...
fn test_partial_eq() {
    let a = ArraySeqStEphS::from_vec(vec![1, 2, 3]);
    let b = ArraySeqStEphS::from_vec(vec![1, 2, 3]);
    let c = ArraySeqStEphS::from_vec(vec![1, 2, 4]);
    assert_eq!(a, b);
    assert_ne!(a, c);
}
...
#[test]
fn test_map() {
    let seq = ArraySeqStEphS::from_vec(vec![1, 2, 3, 4]);
...
#[test]
fn test_tabulate() {
    let seq = ArraySeqStEphS::tabulate(&|i| i * 3, 4);
