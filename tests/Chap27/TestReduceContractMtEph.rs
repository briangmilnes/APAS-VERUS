...
fn test_reduce_contract_parallel_product() {
    ...
    let result = ArraySeqMtEphS::reduce_contract_parallel(&a, Arc::new(|x: &usize, y: &usize| x * y), Ghost::assume_new(), 1);
...
fn test_reduce_contract_parallel_max() {
    ...
    let result =
        ArraySeqMtEphS::reduce_contract_parallel(&a, Arc::new(|x: &usize, y: &usize| if x > y { *x } else { *y }), Ghost::assume_new(), 0);
...
fn test_reduce_contract_parallel_min() {
    ...
    let result = ArraySeqMtEphS::reduce_contract_parallel(
        &a,
        Arc::new(|x: &usize, y: &usize| if x < y { *x } else { *y }),
        Ghost::assume_new(),
        usize::MAX,
    );
    assert_eq!(result, 0);
