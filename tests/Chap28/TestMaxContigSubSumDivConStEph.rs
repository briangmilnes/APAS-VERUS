...
fn test_empty() {
    ...
    let result = ArraySeqStEphS::max_contig_sub_sum_divcon(&a);
...
fn test_crossing_middle() {
    // Specifically tests case where max subsequence crosses the middle
    let a: ArraySeqStEphS<i32> = ArraySeqStEphSLit![2, 3, -1, 4];
    let result = ArraySeqStEphS::max_contig_sub_sum_divcon(&a);
    assert_eq!(result, Some(8)); // [2, 3, -1, 4]
}
