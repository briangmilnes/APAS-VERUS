...
fn quick_sort_mt_variants_produce_sorted_output() {
    ...
    ArraySeqMtEphS::quick_sort_first(&mut first);
    assert_eq!(to_vec(&first), expected);

    let mut median3 = base.clone();
    ArraySeqMtEphS::quick_sort_median3(&mut median3);
    assert_eq!(to_vec(&median3), expected);

    let mut random = base.clone();
    ArraySeqMtEphS::quick_sort_random(&mut random);
...
fn quick_sort_mt_large_inputs() {
    ...
    ArraySeqMtEphS::quick_sort_first(&mut descending);
...
fn quick_sort_mt_concurrent_execution() {
    ...
    handles.push(thread::spawn(move || {
        ...
        ArraySeqMtEphS::quick_sort_first(&mut seq);
        to_vec(&seq)
    }));
    ...
    handles.push(thread::spawn(move || {
        ...
        ArraySeqMtEphS::quick_sort_median3(&mut seq);
        to_vec(&seq)
    }));
    ...
    handles.push(thread::spawn(move || {
        ...
        ArraySeqMtEphS::quick_sort_random(&mut seq);
        to_vec(&seq)
    }));
