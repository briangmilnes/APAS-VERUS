...
fn test_tabulate_inorder() {
    let t: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![0, 1, 2, 3, 4];
    assert_eq!(t.to_arrayseq(), ArraySeqStPerS::from_vec(vec![0, 1, 2, 3, 4]));
}

#[test]
fn test_map_increment() {
    let _base: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![0, 1, 2, 3];
    let mapped: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![1, 2, 3, 4];
    assert_eq!(mapped.to_arrayseq(), ArraySeqStPerS::from_vec(vec![1, 2, 3, 4]));
}

#[test]
fn test_append_union() {
    let _a: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![0, 1, 2, 3];
    let _b: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![2, 3, 4, 5];
    let appended: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![0, 1, 2, 3, 2, 3, 4, 5];
...
#[test]
fn test_filter_even() {
    let _base: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![0, 1, 2, 3, 4, 5];
    let evens: AVLTreeSeqStPerS<usize> = AVLTreeSeqStPerLit![0, 2, 4];
