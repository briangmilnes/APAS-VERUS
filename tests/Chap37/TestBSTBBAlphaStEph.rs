...
fn test_balancing_ascending() {
    ...
    assert!(bst_balance_check::height_within_wb_bound(tree.size(), tree.height()));
}
...
// Balance workloads: the weight-balanced shape is checked after every operation
// from the pre-order traversal alone (tests/Chap37/bst_balance_check.rs).

mod bst_balance_check;
use bst_balance_check::*;

/// Checks the shape of `tree` against its size, panicking with `context` on failure.
fn st_check(tree: &BSTBBAlphaStEph<i64>, context: &str) -> Shape {
    let shape = check_weight_balanced(&tree.pre_order())
        .unwrap_or_else(|e| panic!("{}: {}", context, e));
    assert_eq!(shape.size, tree.size(), "{}: size", context);
    assert!(height_within_wb_bound(tree.size(), tree.height()), "{}: height", context);
    shape
}
...
fn st_insert_checked(mut tree: BSTBBAlphaStEph<i64>, keys: &[i64]) -> (BSTBBAlphaStEph<i64>, usize) {
...
fn st_delete_checked(mut tree: BSTBBAlphaStEph<i64>, keys: &[i64]) -> BSTBBAlphaStEph<i64> {
    for &k in keys {
...
#[test]
fn test_bb_balance_ascending() { st_insert_checked(BSTBBAlphaStEph::new(), &ascending(2000)); }

#[test]
fn test_bb_balance_descending() { st_insert_checked(BSTBBAlphaStEph::new(), &descending(2000)); }

#[test]
fn test_bb_balance_zigzag() { st_insert_checked(BSTBBAlphaStEph::new(), &zigzag(2000)); }

#[test]
fn test_bb_balance_random() {
    for seed in 1..=4 { st_insert_checked(BSTBBAlphaStEph::new(), &random_permutation(2000, seed)); }
}

#[test]
fn test_bb_balance_duplicates() {
    st_insert_checked(BSTBBAlphaStEph::new(), &random_keys(2000, 100, 7));
}

#[test]
fn test_bb_delete_ascending_to_empty() {
    let (tree, _) = st_insert_checked(BSTBBAlphaStEph::new(), &random_permutation(1000, 3));
    let tree = st_delete_checked(tree, &ascending(1000));
    assert!(tree.is_empty());
}

#[test]
fn test_bb_delete_descending_to_empty() {
    let (tree, _) = st_insert_checked(BSTBBAlphaStEph::new(), &ascending(1000));
    let tree = st_delete_checked(tree, &descending(1000));
    assert!(tree.is_empty());
}

#[test]
fn test_bb_delete_random_to_empty() {
    for seed in 1..=3 {
        let (tree, _) = st_insert_checked(BSTBBAlphaStEph::new(), &ascending(1000));
...
#[test]
fn test_bb_delete_absent_keys() {
    let evens: Vec<i64> = (0..500).map(|i| 2 * i).collect();
    let odds: Vec<i64> = (0..500).map(|i| 2 * i + 1).collect();
    let (tree, _) = st_insert_checked(BSTBBAlphaStEph::new(), &evens);
...
#[test]
fn test_bb_insert_delete_interleaved() {
    let mut rng = SeededRng::new(99);
    let mut tree = BSTBBAlphaStEph::<i64>::new();
...
fn test_bb_balance_report() {
    for (name, keys) in [
        ...
        let (_, h) = st_insert_checked(BSTBBAlphaStEph::new(), &keys);
        println!("BB StEph {:<11} n=2000 max height {}", name, h);
    }
}
