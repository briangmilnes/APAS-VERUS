...
fn test_trait_impl_empty() {
    let set: BSTSetPlainMtEph<i32> = <BSTSetPlainMtEph<i32> as BSTSetPlainMtEphTrait<i32>>::empty();
    assert_eq!(set.size(), 0);
}

#[test]
fn test_trait_impl_singleton() {
    let set = <BSTSetPlainMtEph<i32> as BSTSetPlainMtEphTrait<i32>>::singleton(42);
    assert_eq!(set.size(), 1);
    assert!(set.contains(&42));
}
