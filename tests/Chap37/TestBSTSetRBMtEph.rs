// SPDX-License-Identifier: MIT
// Copyright (c) 2026 Umut Acar, Guy Blelloch and Brian Milnes
//! Tests for BSTSetRBMtEph.

use apas_verus::BSTSetRBMtEphLit;
use apas_verus::Chap18::ArraySeqStPer::ArraySeqStPer::ArraySeqStPerBaseTrait;
use apas_verus::Chap37::BSTRBMtEph::BSTRBMtEph::BSTRBMtEphTrait;
use apas_verus::Chap37::BSTSetRBMtEph::BSTSetRBMtEph::*;
use apas_verus::Types::Types::*;

#[test]
fn test_bstsetrbmtephlit_macro_functionality() {
    let empty: BSTSetRBMtEph<i32> = BSTSetRBMtEphLit![];
    assert_eq!(empty.size(), 0);

    let with_data: BSTSetRBMtEph<i32> = BSTSetRBMtEphLit![5, 3, 7, 1, 9];
    assert_eq!(with_data.size(), 5);
    assert_eq!(with_data.find(&5), Some(5));
    assert_eq!(with_data.find(&3), Some(3));
    assert_eq!(with_data.find(&7), Some(7));
    assert_eq!(with_data.find(&10), None);
}

#[test]
fn test_empty() {
    let set = BSTSetRBMtEph::<i32>::empty();
    assert_eq!(set.size(), 0);
    assert!(set.is_empty());
}

#[test]
fn test_singleton() {
    let set = BSTSetRBMtEph::singleton(42);
    assert_eq!(set.size(), 1);
    assert!(set.contains(&42));
}

#[test]
fn test_insert_and_contains() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    set.insert(3).unwrap();
    set.insert(7).unwrap();
    assert_eq!(set.size(), 3);
    assert!(set.contains(&5));
    assert!(set.contains(&3));
    assert!(!set.contains(&10));
}

#[test]
fn test_delete() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    set.insert(3).unwrap();
    set.insert(7).unwrap();
    set.delete(&3);
    assert_eq!(set.size(), 2);
    assert!(!set.contains(&3));
}

#[test]
fn test_minimum_maximum() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    set.insert(3).unwrap();
    set.insert(7).unwrap();
    set.insert(1).unwrap();
    set.insert(9).unwrap();
    assert_eq!(set.minimum(), Some(1));
    assert_eq!(set.maximum(), Some(9));
}

#[test]
fn test_union() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    set1.insert(2).unwrap();
    let mut set2 = BSTSetRBMtEph::empty();
    set2.insert(2).unwrap();
    set2.insert(3).unwrap();
    let union = set1.union(&set2);
    assert_eq!(union.size(), 3);
    assert!(union.contains(&1));
    assert!(union.contains(&2));
    assert!(union.contains(&3));
}

#[test]
fn test_intersection() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    set1.insert(2).unwrap();
    set1.insert(3).unwrap();
    let mut set2 = BSTSetRBMtEph::empty();
    set2.insert(2).unwrap();
    set2.insert(3).unwrap();
    set2.insert(4).unwrap();
    let inter = set1.intersection(&set2);
    assert_eq!(inter.size(), 2);
    assert!(inter.contains(&2));
    assert!(inter.contains(&3));
}

#[test]
fn test_difference() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    set1.insert(2).unwrap();
    set1.insert(3).unwrap();
    let mut set2 = BSTSetRBMtEph::empty();
    set2.insert(2).unwrap();
    let diff = set1.difference(&set2);
    assert_eq!(diff.size(), 2);
    assert!(diff.contains(&1));
    assert!(diff.contains(&3));
}

#[test]
fn test_split() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(1).unwrap();
    set.insert(3).unwrap();
    set.insert(5).unwrap();
    set.insert(7).unwrap();
    let (left, found, right) = set.split(&5);
    assert!(found);
    assert_eq!(left.size(), 2);
    assert_eq!(right.size(), 1);
}

#[test]
fn test_filter() {
    let mut set = BSTSetRBMtEph::empty();
    for i in 1..=10 {
        set.insert(i).unwrap();
    }
    let evens = set.filter(|x| x % 2 == 0);
    assert_eq!(evens.size(), 5);
    assert!(evens.contains(&2));
    assert!(evens.contains(&4));
}

#[test]
fn test_reduce() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(1).unwrap();
    set.insert(2).unwrap();
    set.insert(3).unwrap();
    let sum = set.reduce(|a, b| a + b, 0);
    assert_eq!(sum, 6);
}

#[test]
fn test_iter_in_order() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    set.insert(3).unwrap();
    set.insert(7).unwrap();
    set.insert(1).unwrap();
    let seq = set.iter_in_order();
    assert_eq!(seq.length(), 4);
    assert_eq!(*seq.nth(0), 1);
    assert_eq!(*seq.nth(1), 3);
    assert_eq!(*seq.nth(2), 5);
    assert_eq!(*seq.nth(3), 7);
}

#[test]
fn test_large_set() {
    let mut set = BSTSetRBMtEph::empty();
    for i in 0..100 {
        set.insert(i).unwrap();
    }
    assert_eq!(set.size(), 100);
    assert_eq!(set.minimum(), Some(0));
    assert_eq!(set.maximum(), Some(99));
}

#[test]
fn test_delete_multiple() {
    let mut set = BSTSetRBMtEph::empty();
    for i in 0..10 {
        set.insert(i).unwrap();
    }
    set.delete(&5);
    set.delete(&3);
    set.delete(&7);
    assert_eq!(set.size(), 7);
    assert!(!set.contains(&5));
}

#[test]
fn test_duplicate_insert() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    let _ = set.insert(5);
    let _ = set.insert(5);
    assert_eq!(set.size(), 1);
}

#[test]
fn test_balanced_after_inserts() {
    let mut set = BSTSetRBMtEph::empty();
    for i in 1..=100 {
        set.insert(i).unwrap();
    }
    // Red-Black tree should stay balanced
    assert_eq!(set.size(), 100);
    assert!(set.contains(&50));
}

#[test]
fn test_union_empty() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    let set2 = BSTSetRBMtEph::empty();
    let union = set1.union(&set2);
    assert_eq!(union.size(), 1);
}

#[test]
fn test_intersection_disjoint() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    set1.insert(2).unwrap();
    let mut set2 = BSTSetRBMtEph::empty();
    set2.insert(3).unwrap();
    set2.insert(4).unwrap();
    let inter = set1.intersection(&set2);
    assert_eq!(inter.size(), 0);
}

#[test]
fn test_negative_numbers() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(-5).unwrap();
    set.insert(-3).unwrap();
    set.insert(-7).unwrap();
    set.insert(0).unwrap();
    assert_eq!(set.minimum(), Some(-7));
    assert_eq!(set.maximum(), Some(0));
}

#[test]
fn test_join_pair() {
    let mut left = BSTSetRBMtEph::empty();
    left.insert(1).unwrap();
    left.insert(2).unwrap();
    let mut right = BSTSetRBMtEph::empty();
    right.insert(5).unwrap();
    right.insert(6).unwrap();
    let joined = BSTSetRBMtEph::join_pair(left, right);
    assert_eq!(joined.size(), 4);
}

#[test]
fn test_join_m() {
    let mut left = BSTSetRBMtEph::empty();
    left.insert(1).unwrap();
    left.insert(2).unwrap();

    let mut right = BSTSetRBMtEph::empty();
    right.insert(6).unwrap();
    right.insert(7).unwrap();

    let joined = BSTSetRBMtEph::join_m(left, 5, right);
    assert_eq!(joined.size(), 5);
    assert!(joined.contains(&1));
    assert!(joined.contains(&2));
    assert!(joined.contains(&5));
    assert!(joined.contains(&6));
    assert!(joined.contains(&7));
}

#[test]
fn test_join_m_with_empty() {
    let empty = BSTSetRBMtEph::empty();
    let mut right = BSTSetRBMtEph::empty();
    right.insert(6).unwrap();
    right.insert(7).unwrap();

    let joined = BSTSetRBMtEph::join_m(empty, 5, right);
    assert_eq!(joined.size(), 3);
    assert!(joined.contains(&5));
    assert!(joined.contains(&6));
    assert!(joined.contains(&7));
}

#[test]
fn test_as_tree() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    set.insert(3).unwrap();
    set.insert(7).unwrap();

    let tree = set.as_tree();
    assert_eq!(tree.size(), 3);
    assert!(tree.contains(&5));
}

#[test]
fn test_trait_impl_empty() {
    let set: BSTSetRBMtEph<i32> = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::empty();
    assert_eq!(set.size(), 0);
}

#[test]
fn test_trait_impl_singleton() {
    let set = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::singleton(42);
    assert_eq!(set.size(), 1);
    assert!(set.contains(&42));
}

#[test]
fn test_trait_impl_contains() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(10).unwrap();
    assert!(<BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::contains(&set, &10));
}

#[test]
fn test_trait_impl_find() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(20).unwrap();
    assert_eq!(
        <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::find(&set, &20),
        Some(20)
    );
}

#[test]
fn test_trait_impl_minimum() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    set.insert(3).unwrap();
    set.insert(7).unwrap();
    assert_eq!(<BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::minimum(&set), Some(3));
}

#[test]
fn test_trait_impl_maximum() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();
    set.insert(3).unwrap();
    set.insert(7).unwrap();
    assert_eq!(<BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::maximum(&set), Some(7));
}

#[test]
fn test_trait_impl_insert() {
    let mut set = BSTSetRBMtEph::empty();
    <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::insert(&mut set, 15).unwrap();
    assert_eq!(set.size(), 1);
}

#[test]
fn test_trait_impl_delete() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(10).unwrap();
    set.insert(20).unwrap();
    <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::delete(&mut set, &10);
    assert_eq!(set.size(), 1);
    assert!(!set.contains(&10));
}

#[test]
fn test_trait_impl_union() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    let mut set2 = BSTSetRBMtEph::empty();
    set2.insert(2).unwrap();

    let result = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::union(&set1, &set2);
    assert_eq!(result.size(), 2);
}

#[test]
fn test_trait_impl_intersection() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    set1.insert(2).unwrap();
    let mut set2 = BSTSetRBMtEph::empty();
    set2.insert(2).unwrap();
    set2.insert(3).unwrap();

    let result = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::intersection(&set1, &set2);
    assert_eq!(result.size(), 1);
    assert!(result.contains(&2));
}

#[test]
fn test_trait_impl_difference() {
    let mut set1 = BSTSetRBMtEph::empty();
    set1.insert(1).unwrap();
    set1.insert(2).unwrap();
    let mut set2 = BSTSetRBMtEph::empty();
    set2.insert(2).unwrap();

    let result = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::difference(&set1, &set2);
    assert_eq!(result.size(), 1);
    assert!(result.contains(&1));
}

#[test]
fn test_trait_impl_split() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(1).unwrap();
    set.insert(5).unwrap();
    set.insert(10).unwrap();

    let (left, found, right) = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::split(&set, &5);
    assert!(found);
    assert_eq!(left.size(), 1);
    assert_eq!(right.size(), 1);
}

#[test]
fn test_trait_impl_join_pair() {
    let mut left = BSTSetRBMtEph::empty();
    left.insert(1).unwrap();
    let mut right = BSTSetRBMtEph::empty();
    right.insert(5).unwrap();

    let result = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::join_pair(left, right);
    assert_eq!(result.size(), 2);
}

#[test]
fn test_trait_impl_join_m() {
    let mut left = BSTSetRBMtEph::empty();
    left.insert(1).unwrap();
    let mut right = BSTSetRBMtEph::empty();
    right.insert(5).unwrap();

    let result = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::join_m(left, 3, right);
    assert_eq!(result.size(), 3);
    assert!(result.contains(&3));
}

#[test]
fn test_trait_impl_filter() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(1).unwrap();
    set.insert(2).unwrap();
    set.insert(3).unwrap();

    let result = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::filter(&set, |x| x % 2 == 1);
    assert_eq!(result.size(), 2);
}

#[test]
fn test_trait_impl_reduce() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(1).unwrap();
    set.insert(2).unwrap();
    set.insert(3).unwrap();

    let sum = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::reduce(&set, |a, b| a + b, 0);
    assert_eq!(sum, 6);
}

#[test]
fn test_trait_impl_iter_in_order() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(3).unwrap();
    set.insert(1).unwrap();
    set.insert(2).unwrap();

    let seq = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::iter_in_order(&set);
    assert_eq!(seq.length(), 3);
}

#[test]
fn test_trait_impl_as_tree() {
    let mut set = BSTSetRBMtEph::empty();
    set.insert(5).unwrap();

    let tree = <BSTSetRBMtEph<i32> as BSTSetRBMtEphTrait<i32>>::as_tree(&set);
    assert_eq!(tree.size(), 1);
}
