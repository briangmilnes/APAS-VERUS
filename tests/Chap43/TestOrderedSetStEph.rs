...
#[test]
fn test_singleton() {
    let set = OrderedSetStEph::singleton(42);
...
fn test_ephemeral_semantics() {
    let mut original = OrderedSetStEph::empty();
    original.insert(1);
    original.insert(2);
    original.insert(3);
...
#[test]
fn test_ordered_set_st_eph_lit_macro() {
    let set: OrderedSetStEph<i32> = OrderedSetStEphLit![1, 3, 5, 7, 9];
    assert_eq!(set.size(), 5);
    assert!(set.find(&1));
    assert!(set.find(&3));
    assert!(set.find(&5));
